<template>
	<view>
		<scroll-view @click="doBtn('toolHide')" @scroll="doScroll" :scroll-top="canTop * -1 + 'px'" :scroll-left="canLeft * -1 + 'px'" scroll-x="true" scroll-y="true" class="container" :style="{width: '100vw', height: '100vh'}">
			<canvas  canvas-id="myCanvas" id="myCanvas" :style="{width:canWidth + 'px',height:canHeight + 'px',display:canShow?'':'none'}"></canvas>
		</scroll-view>
		<canvas canvas-id="canDummy" id="myCanvas" :style="{width:canDummyWidth + 'px',height:canDummyHeight + 'px',display:canDummyShow?'':'none'}"></canvas>
		<view id="meImage" :style="{left:meLeft + 'px',top:meTop + 'px',display:meDisp,'z-index':turnShow?5001:1}">
			<image mode="aspectFit" src="/static/me.png" :style="{width:'40rpx',height:'40rpx',transform:'rotate('+rotateMe+'deg);','border-radius':'50%'}"></image>
			<text>{{currentDistance}}cm</text>
		</view>
		<view @tap="doBtn('turn')" style="position:fixed;top:20rpx;right:20rpx;text-align:right;">
			<image class='m-compassbg' src='/static/compass.png' :style='"transform:rotate("+rotate+"deg);"'></image>
		</view>
		<image v-if="overviewShow" @tap="doBtn('overview')" mode="aspectFit" id="fullImage" :src="canvasImage"></image>
		<view class="sidebar" v-if="sidebarShow">
			...
			<image v-for="item in toolsFlag" :key="item.id" @tap="doBtn('tf_' + item.id)" class="toolImage" :src="item.toolIcon" />
		...
		<view class="toolsBottom">
			<!-- 直线/弧线 -->
			<view>
				<image v-if="drawingArc" @tap="doBtn('arc')" class="toolImage" src="/static/curve_white.png" style="background:#106CFF;" />
				<image v-else @tap="doBtn('arc')" class="toolImage" src="/static/straight_line_white.png" style="background:#106CFF;" />
			</view>
			<!-- 结构类型 -->
			<view>
				<view class="toolsLine" v-if="toolsLineShow">
					<block v-for="item in toolsLine"  v-bind:key="item.id">
						<image @tap="doBtn('tl_' + item.id);" :class="item.class ? 'toolImage toolBtnActive' : 'toolImage'" :src="item.class ? item.iconWhite : item.icon" :style="item.icon.indexOf('parapet') != -1 ? 'padding:5px 4px 4px 8px' : ''" />
					</block>
				</view>
				<image @tap="doBtn('toolsLineShow')" class="toolImage" src="/static/wall_white.png" style="background:#106CFF;padding:5px 7px;" />
			</view>
			<!-- 开始/暂停 -->
			<view>
				<image v-if="pause" @tap="doBtn('start')" class="toolImage" src="/static/play_white.png" style="width:140rpx;height:140rpx;padding:10px 6px 10px 14px;background:#106CFF;" />
				<image v-else @tap="doBtn('start')" class="toolImage" src="/static/pause_white.png" style="width:140rpx;height:140rpx;background:#106CFF;" />
			</view>
			<!-- 设置 -->
			<view>
				<view class="more" v-if="moreShow">
					...
					<text @tap="doBtn('initBlue')" class="toolImage toolText" :class="blueIndex >= 0 ? 'toolBtnActive':''">蓝牙</text>
					<image @tap="doBtn('setting');" class="toolImage" src="/static/setUp.png" />
				</view>
				<image @tap="doBtn('moreShow')" class="toolImage" src="/static/more_white.png" style="background:#106CFF;padding:0;" />
			</view>
			<!-- 调试 -->
			<view>
				<view class="debug" v-if="debugShow">
					<text @tap="doBtn('golog')"  class="toolImage toolText" :class="showLog ? 'toolBtnActive':''">日志</text>
					<text  @tap="doBtn('gensui')" class="toolImage toolText toolBtnActive">{{gensui? '跟随':'固定'}}</text>
					<text @tap="doBtn('curPosition')"  class="toolImage toolText">当前</text>
          <text @tap="doBtn('huaxian')" class="toolImage toolText toolBtnActive">{{drawing? '画图':'移动' }}</text>
          <text @tap="doBtn('addRadius')" class="toolImage toolText">半径</text>
					<text @tap="doBtn('doLockDegree')"  class="toolImage toolText" :class="lockDegree != -1 ? 'toolBtnActive':''">锁角</text>
					<text @tap="doBtn('dummy')"  class="toolImage toolText" :class="dummyEnable ? 'toolBtnActive':''">模拟</text>
				</view>
				<image @tap="doBtn('debugShow')" class="toolImage" src="/static/debug_white.png" style="background:#106CFF;padding:10px;" />
			</view>
		</view>
    <mp-half-screen-dialog
		...
		>
			<view slot="desc">
        <view v-for="d in bdata" v-bind:key="d.id" style="height:100;overflow-y:auto;">{{d.id}} -> {{d.v}}</view>
			</view>
		</mp-half-screen-dialog>
		<mp-half-screen-dialog
		...
		>
			<view slot="desc">
				<textarea :value="statisticTxt" :disabled="true" :maxlength="-1" style="width:100%;height:400px"></textarea>
			</view>
		</mp-half-screen-dialog>
		<mp-half-screen-dialog 
		...
		>
			<view slot="desc">
				<uni-number-box :min="0" :max="9999" :value="curFanVal" class="uni-number-box"  @change="fanChange" />
			</view>
		</mp-half-screen-dialog>
		<mp-half-screen-dialog 
			@buttontap="goConfirm"
...
<script>
	export default {
		...
		onReady() {
			...
			if(obj)
				this.deviation = parseInt(obj);
			obj = uni.getStorageSync('markDegree');
			if(obj)
				this.markDegree = parseInt(obj);
			obj = uni.getStorageSync('distance');
			if(obj)
				this.dummyDistanceStep = parseInt(obj);
			...
			if(obj)
				this.markTxtSize = parseInt(obj);
			obj = uni.getStorageSync('startPointGap');
			if(obj)
				this.startPointGap = parseInt(obj);
			obj = uni.getStorageSync('wheelRadius');
			if(obj)
				this.wheelRadius = parseInt(obj);
    ...
    onUnload() {
      ...
      if(this.blueOpen) this.doInitBlue();
		...
		methods:{
			...
			doInterEvent() {
				if(this.dummyEnable)
					this.doDraw(this.dummyDistanceStep);
			},
			/**
			 * 手动拖拽画布位置滚动事件
			...
			doExportImage(func, useDummy, rightDummy) {
				if(!func)
					return;
				let minWidth = 300, minHeight = 300;
				let _this = this, margin = 20;
					...
					success: function(res) {
						...
						if(_this.statLast) {
							...
							for(var tl in _this.statLast.toolsLineCur) {
								if(!_this.contains(toolsLineIds, tl))
									toolsLineIds.push(tl);
							}
						}
						// 绘制使用的图标图例
						...
						if(rightDummy) {
							if(toolsLineIds.length > 0)
								cWidth += 120;
							if(toolsFlagIds.length > 0)
								cWidth += 120;
						}else {
							var addHeight = 0;
							if(toolsLineIds.length > 3)
								addHeight = (toolsLineIds.length - 3) * 22;
							if(toolsFlagIds.length > 3)
								addHeight = (toolsFlagIds.length - 3) * 25;
							cHeight += addHeight;
						}
						
						if(rightDummy && cWidth < 220)
							cWidth = 220;
						...
						_this.doDrawImage(_this.ctxDummy, res.tempFilePath, 0, rightDummy?0:cHeight, width, height);
						if(rightDummy)
							_this.doDrawLine(_this.ctxDummy, rightDummyOffset - 2, 0, rightDummyOffset - 2, height, '#bfbfbf', 1, true);
						else
							_this.doDrawLine(_this.ctxDummy, 0, cHeight - 12, width, cHeight - 12, '#bfbfbf', 1, true);
						// 绘制指北针图标
						_this.doDrawImage(_this.ctxDummy, _this['icon__north'], width + cWidth - 60, 10, 50, 50);
						// 绘制文本，单位:cm
						_this.doDrawText(_this.ctxDummy, '单位:cm',{x:width + cWidth - 60,y:86}, 12, '#404040');
						// 比例文本输出
						...
						if(rightDummy) {
							...
							var lineY = 106, lineHeight = 20;

							function _doDrawStatToDummy(txt, leftTab) {
								var xp = lineX;
								if(leftTab && leftTab > 0)
									xp += leftTab;
							...
							if(data.graphs) {
								for(var i = 0;i < data.graphs.length; i++) {
									...
									if(graph.area == -1)
										_doDrawStatToDummy('面积：未闭环' + fanTxt, 20);
									else
										_doDrawStatToDummy('面积：' + ((graph.area + fanArea) / 10000).toFixed(2) + '平米' + fanTxt, 20);
									_doDrawStatToDummy('边/角数量：' + graph.edgeCount, 20);
									_doDrawStatToDummy('总边长：' + graph.edgeLength, 20);
									
									for(var j = 0;j < graph.arcs.length; j++)
										_doDrawStatToDummy('弧'+graph.arcs[j].id+'面积：' + (graph.arcs[j].area / 10000).toFixed(2), 20);
										
									if(graph.lines) {
										_doDrawStatToDummy('线条数量：' + graph.lineCount, 20);
										for(var j = 0;j < graph.lines; j++)
											_doDrawStatToDummy(graph.lines[i].name+'数量：' + graph.lines[i].count + ';长度：' + graph.lines[i].length, 40);
									}
									
									if(graph.fans) {
										_doDrawStatToDummy('上/下翻线条数量：' + graph.fans.length, 20);
										for(var j = 0;j < graph.fans.length; j++)
											_doDrawStatToDummy(graph.fans[j].name+'上/下翻：' + graph.fans[j].fan + ';长度：' + graph.fans[j].dis, 40);
									}
								}
							}
							if(data.lines) {
								...
								for(var i = 0;i < data.lines.length; i++)
									_doDrawStatToDummy(data.lines[i].name+'数量：' + data.lines[i].count + ';长度：' + data.lines[i].length, 20);
							}
							if(data.flags) {
						...
						for(var i = 0;i < toolsLineIds.length; i++) {
							for(var j = 0;j < _this.toolsLine.length; j++) {
								...
								if(tl.id == toolsLineIds[i]) {
									_this.doDrawText(_this.ctxDummy, _this.toolsLine[j].name,{x:54 + rightDummyOffset,y:10 + 20*i}, 12, '#f1aa00');
									let lwidth = _this.defaultLineWidth;
									let lcolor = _this.defaultLineColor;
									let loffset = 0;
									if(tl.type == 'line') { // 主线条变化
										if(tl.width)
											lwidth = tl.width;
										if(tl.color)
											lcolor = tl.color;
									}else if(tl.type == 'lline' || tl.type == 'rline') { // 左侧/右侧线条
										let lgap = 3, color = _this.defaultLineColor, width = _this.defaultLineWidth;
										if(tl.gap)
											lgap = tl.gap;
										if(tl.color)
											color = tl.color;
										...
										if(tl.type == 'lline')
											_this.doDrawLine(_this.ctxDummy, 10 + rightDummyOffset, 4 + 20 * i + lgap, 50 + rightDummyOffset, 4 + 20 * i + lgap, color, width);
										else {
											_this.doDrawLine(_this.ctxDummy, 10 + rightDummyOffset, 4 + 20 * i, 50 + rightDummyOffset, 4 + 20 * i, color, width);
											loffset = lgap;
										}
									}
									
									// 绘制主线条
									_this.doDrawLine(_this.ctxDummy, 10 + rightDummyOffset, 4 + 20 * i + loffset, 50 + rightDummyOffset, 4 + 20 * i + loffset, lcolor, lwidth);
									continue;
								}
							}
						}
						...
						for(var i = 0;i < toolsFlagIds.length; i++) {
							for(var j = 0;j < _this.toolsFlag.length; j++) {
								...
								if(tf.id == toolsFlagIds[i]) {
									_this.doDrawImage(_this.ctxDummy, _this['icon_' + tf.id], 130 + rightDummyOffset - (toolsLineIds.length == 0?120:0), 0 + 24 * i, 20, 20);
									_this.doDrawText(_this.ctxDummy, tf.name,{x:154 + rightDummyOffset - (toolsLineIds.length == 0?120:0),y:14 + 24*i}, 12, '#f1aa00');
									continue;
								}
							}
						}
			...
			doBtn(id) {
				...
				function _moveAdjusted() {
					...
					if(obj)
						_this.markDegree = parseInt(obj);
					else
						_this.markDegree = 45;
					obj = uni.getStorageSync('deviation');
					if(obj)
						_this.deviation = parseInt(obj);
					else
						_this.deviation = 15;
				}
				...
				if(id == 'dummy') {
					...
					this.moreShow = false;
					this.debugShow = false;
				}else if(id == 'moreShow'){ // 展示/隐藏更多工具栏
					this.moreShow = !this.moreShow;
					this.toolsLineShow = false;
					this.debugShow = false;
				}else if(id == 'debugShow'){ // 展示/隐藏调试工具栏
					this.debugShow = !this.debugShow;
					this.toolsLineShow = false;
					this.moreShow = false;
				}else if(id == 'toolHide'){
					...
					this.moreShow = false;
					this.debugShow = false;
				}else if(id.startsWith('tl_')) { // 线条工具按钮
					id = id.substring(3);
					for(let i = 0;i < this.toolsLine.length; i++) {
						if(this.toolsLine[i].id == id) {
							if(!this.toolsLine[i].class) {
								...
								if(this.toolsLine[i].fan == 1) { // 当前线条支持上/下翻，则弹出设置窗口
									this.curFanVal = 0;
									this.fanShow = true;
								}
							}else {
								if(this.toolsLine[i].fan == 1) // 当前线条支持上/下翻，则重置距离值为0
									this.curFanVal = 0;
				...
				}else if(id.startsWith('tf_')) { // 标记工具按钮
					id = id.substring(3);
					for(let i = 0;i < this.toolsFlag.length; i++) {
						...
						if(tf.id == id) {
							...
							if(this.statLast)
								this.statLast.toolsFlag.push(tfdata);
							this.statToolsFlag.push(tfdata);
							if(this.maxX < this.x + w)
								this.maxX = this.x + w;
							if(this.maxY < this.y + h)
								this.maxY = this.y + h;
							break;
						}
					}
				}else if(id == 'gensui') { // 跟随
					this.gensui = !this.gensui;
					if(this.gensui)
						this.doCurrentPosition();
				}else if(id == 'huaxian') { // 划线/移动
					this.drawing = !this.drawing;
					if(this.drawing) {
						// 切换回划线模式绘制新起点
						if(this.x != this.startPointX || this.y != this.startPointY)
				...
				}else if(id == 'arc'){ // 绘制弧线/直线切换
					if(!this.drawingArc) { // 开始绘制弧线
				...
				}else if(id == 'noteSave') { // 标注
					this.doDrawText(this.ctx, this.noteContent,{x:this.x,y:this.y}, this.markTxtSize, '#f1aa00');
					this.noteContent = '';
					// this.$refs.popupNote.close();
					this.noteShow = false;
					this.onDialogChange(this.noteShow);
				}else if(id == 'fanShow'){ // 移动指定距离
					this.fanShow = !this.fanShow;
					this.onDialogChange(this.fanShow);
				}else if(id == 'goShow'){ // 移动指定距离
					this.goShow = !this.goShow;
					this.onDialogChange(this.goShow);
				}else if(id == 'popupStatisticClose') { // 关闭统计popup
					// this.$refs.popupStatistic.close();
					this.statisticShow = false;
					this.onDialogChange(this.statisticShow);
				}else if(id == 'popupNoteClose') { // 关闭标注popup
					// this.$refs.popupNote.close();
					this.noteShow = false;
					this.onDialogChange(this.noteShow);
				}else if(id == 'doLockDegree') { // 锁定角度按钮
			...
			doCurrentPosition() {
				...
				setTimeout(function(){
					...
					if(_this.canLeft == cleft) // 如相等则会失效，因此加1
						cleft += 1;
					_this.canLeft = cleft;
					setTimeout(function(){
						let ctop = _this.windowHeight / 2 - _this.y;
						if(_this.canTop == ctop) // 如相等则会失效，因此加1
							ctop += 1;
						_this.canTop = ctop;
					}, 100);
				}, 100);
			...
			doRedraw() {
				var _this = this;
				var opp = this.cmPerPx, npp = opp + 1;
				if(opp > 1) // 确保可被整除的偶数倍数
					npp = opp + 2;
				var scale = opp / npp;
				this.cmPerPx = npp;
				// 清空画布
				...
				function _drawStat(st, doMark) {
					...
					let lwidth = _this.defaultLineWidth;
					let lcolor = _this.defaultLineColor;
					console.log(st.x + ':' + nx + ";" + st.y + ":" + ny + ";" + st.ex + ":" + nex + ':' + st.ey + ":" + ney);
					st.x = nx;
					st.y = ny;
					if(st.ex)
						st.ex = nex;
					...
					var toolsLineList = [];
					for(var i = 0; i < st.toolsLine.length;i++)
						toolsLineList.push(st.toolsLine[i]);
					for(var tln in st.toolsLineCur)
						toolsLineList.push(st.toolsLineCur[tln]);
						...
						for(let i = 0;i < _this.toolsLine.length; i++) {
							...
							if(tl.id == stl.id) {
								...
								if(tl.type == 'line') { // 主线条变化
									...
									var dir = stl.dir, dirGap = stl.dirGap;
									...
									_this.doDrawLine(_this.ctx, stl.x + gapx * q, stl.y + gapy * q, stl.ex + gapx * q, stl.ey + gapy * q, color, width);
								}
							}
						}
					}
				...
				for(var i = 0;i < this.stat.length; i++) {
					var st = this.stat[i];
					if(i == 0)
						this.doDrawPoint(st.x, st.y, 4, '#00ff00', false);
					_drawStat(st, true);
				}
			...
			doDraw(dd, ignorePause) {
				...
				// 起点闭环后则停止绘画
				if(this.drawing && this.drawClosed)
					return;
				...
				if(dis <= 0 || dispx <= 0) {
					...
					if(dis > 0 && dispx <= 0)
						this.doAdjustDegree(this.degree);
					return;
				}
				...
				if(this.startPointGap > 0 && this.drawing && sl && this.x == sl.x && this.y == sl.y && sl.degree % 90 == 0) {
				...
				if(this.drawing) {
					...
					for(let i = 0;i < this.toolsLine.length; i++) {
						...
						if(tl.class == this.toolsLineActiveClass) {
							...
							if(tl.type == 'line') { // 主线条变化
								...
								let lgap = 3, color = this.defaultLineColor, width = this.defaultLineWidth;
								if(tl.gap)
									lgap = tl.gap;
								if(tl.color)
									color = tl.color;
								...
								this.doDrawLine(this.ctx, oldx + gapx * q, oldy + gapy * q, this.x + gapx * q, this.y + gapy * q, color, width);
							}
							// 划线工具统计
					...
					// 是否需要缩图
					if(this.x > this.canSize - this.windowWidth / 2 || this.x < this.windowWidth / 2 || 
						this.y > this.canSize - this.windowHeight / 2 || this.y < this.windowHeight / 2)
						this.doRedraw();
				...
				if(this.drawing) { // 画图模式（非移动模式）时才更新图形最大尺寸数据
					if(this.x > this.maxX)
						this.maxX = this.x;
					if(this.x < this.minX)
						this.minX = this.x;
				...
				if(st) { // 画图模式时
					let d = parseInt((this.cmPerPx * this.doCalLineLength(st.x, st.y, this.x, this.y)).toFixed(0));
					this.currentDistance = d;
				}else if(!this.drawing){ // 移动模式时
			...
			doAdjustDegree(degree, undoStat, ignoreLock) {
				// 存在锁定角度则以锁定角度为准
				if(this.lockDegree != -1 && !ignoreLock)
					degree = this.lockDegree;
				let _this = this;
				let cd = 0, deviation = this.deviation, rcd = degree, markDegree = this.markDegree;
				let dd = degree - this.lastDegree;

				if(this.lastDegree != -1 && dd > -1 * deviation && dd < deviation && this.statLast) {
					if(this.statLast.type == 'line' && !this.drawingArc)
						return this.statLast.degree;
				}
				while(cd <= 360) {
				...
				if(this.statLast) {
					let doStat = false, doClear = false;
					if(this.drawing) {
						let slt = this.drawingArc?'arc':'line';
						if((this.statLast.type == 'line' && this.statLast.degree != rcd) || this.statLast.type != slt)
							doStat = true;
					}else {
						// 移动时进行统计，并且清除之前的统计状态
						doStat = true;
					...
					if(doClear) { // 移动之后缓存之前的线段数据，并清理当前线段统计状态
				...
				function _newStatLast(cdegree) {
					...
					if(_this.statLast.type == 'arc')
						_this.statLast.points = [];
			...
			doMarkLine(st) {
				...
				if(!inf || inf.dis == 0)
					return;
				let distance = inf.dis, x = inf.x, y = inf.y, degree = inf.degree;

				// 绘制文本需与直线平行绘制
				let rotateDegree = degree - 90;
				if(rotateDegree < 0)
					rotateDegree = 360 + rotateDegree;
				else if(rotateDegree > 90 && rotateDegree < 270) // 防止倒立数字
					rotateDegree -= 180;
				...
				this.doDrawText(this.ctx, txt,{x:0 - textWidth / 2,y:0 - 5}, this.markTxtSize, '#0000ff');
			...
			doCalInfo(st, drawArcArea) {
				let ex = this.x, ey = this.y, edistance = this.distance, dis = edistance - st.distance;
				if(st.ex)
					ex = st.ex;
				if(st.ey)
					ey = st.ey;
				if(st.edistance)
					edistance = st.edistance;
				if(st.dis)
					dis = st.dis;
				if(st.type == 'arc') {
			...
			doCalArcInfo(st, ex, ey, edistance, dis, drawArcArea) {
				...
				let gap = Number.MAX_VALUE, cp = null;
			...
			doCalArcArea(st, ex, ey, cp, drawArcArea) {
				let x1=st.x,y1=st.y,x2=ex,y2=ey,x3=cp.x,y3=cp.y;
				// 求圆心点坐标
			 ...
			 */
			doArea(stat) {
				let n = stat.length, area = 0.0;
				let logObj = [];
				let minx = this.canSize, miny = this.canSize, maxx = 0, maxy = 0;
				for(let i = 0; i < n; i++) {
				...
				// 居中绘制面积文本
				this.doDrawText(this.ctx, area+'',{x:minx + (maxx - minx) / 2,y:miny + (maxy - miny) / 2}, this.markTxtSize, '#fc0280', true);
			...
			doCloseDraw(closeCurStatLastOnly) {
				...
				for(let i in sl.toolsLineCur)
					sl.toolsLine.push(sl.toolsLineCur[i]);
			...
			doStatistic() {
				...
				if(data.graphs) {
					for(var i = 0;i < data.graphs.length; i++) {
						...
						var fanTxt = '', fanArea = 0;
						// 计算上/下翻面积
						if(graph.fans) {
							fanTxt += '\n\t上/下翻线条数量：' + graph.fans.length;
							for(var j = 0;j < graph.fans.length; j++) {
								fanTxt += '\n\t\t'+graph.fans[j].name+'上/下翻：' + graph.fans[j].fan + ';长度：' + graph.fans[j].dis;
								fanArea += graph.fans[j].fan * graph.fans[j].dis;
							}
						}
						...
						if(graph.lines) {
							...
							for(var j = 0;j < graph.lines; j++)
								this.statisticTxt += '\n\t\t'+graph.lines[j].name+'数量：' + graph.lines[j].count + ';长度：' + graph.lines[j].length;
						}
				...
				if(data.lines) {
					this.statisticTxt += '\n线条数量：' + data.lineCount;
					for(var i = 0;i < data.lines.length; i++)
						this.statisticTxt += '\n\t'+data.lines[i].name+'数量：' + data.lines[i].count + ';长度：' + data.lines[i].length;
				}
				if(data.flags) {
					this.statisticTxt += '\n标记数量：' + data.flagCount;
					for(var i = 0;i < data.flags.length; i++)
			...
			doStatisticData() {
				...
				let stat = [];
				for(var i = 0;i<this.stat.length;i++)
					stat.push(this.stat[i]);
				if(this.statLast)
					stat.push(this.statLast);
				if(stat.length > 0)
				...
				if(lineTotal > 0) {
					...
					data.lines = [];
					for(var f in line)
						data.lines.push({name: f, count: lineCount[f], length: line[f]});
				}
				// 标记统计
				...
				if(stf.length > 0) {
					...
					for(var f in flag)
						data.flags.push({name: f, count: flag[f]});
				}

				function _doStat(i, stat, cline, clineCount, cdt) {
					if(!cdt.graphs)
						cdt.graphs = [];
					let lineTotal = 0, line = {}, lineCount = {}, graph = {};
					...
					for(var i = 0; i < arcs.length; i++)
						graph.arcs.push({id: i + 1, area: arcs[i].area});

					if(lineTotal > 0) {
						graph.lineCount = lineTotal;
						graph.lines = [];
						for(var f in line)
							graph.lines.push({name: f, count: lineCount[f], length: line[f]});
					}
					for(var v in line) {
						if(!cline[v])
							cline[v] = 0;
			...
			compassStart() {
				...
				uni.onCompassChange((res)=>{
					if(!_this.turnHand)
						_this.doTurn(parseInt(res.direction.toFixed(0)));
				});
				uni.startCompass();
				uni.showToast({
					title:'罗盘已启动',
				        ...
				        success(res) {
				            ...
				            uni.onBluetoothDeviceFound(res => {
								...
								for(var i= 0;i < res.devices.length; i++) {
									let device = res.devices[i], memo = '', c = true;
									for(var v in device) {
										if(v == 'connectable') {
											if(!device[v])
												c = false;
								...
								uni.getBLEDeviceCharacteristics({
									...
									success(res) {
										...
										uni.notifyBLECharacteristicValueChange({
													...
													if (len % 2 !== 0) {
														...
														if(result && result.trim()) {
															if(result.indexOf('f1') != -1) {
																...
																dolog("r:" + result + ";a:" + dist.toFixed(1) + ';b:' + _this.blueDistance.toFixed(1) + ';c:' + distCur);
																if(!dist || distCur == 0 || distCur > 2000)
																	return;
																else if(distCur < 0) { // 滚轮后退或重置的情况则重置蓝牙当前距离数据
																...
																if(distCur > 0) {
																	dolog(deg + "度走" + distCur + "cm;ct:" + dist.toFixed(0) + ";lt:" + _this.blueDistance.toFixed(1));
											...
											fail(err) { // notifyBLECharacteristicValueChange
												console.error(err);
												uni.showToast({title:'设备连接失败:4'});
											}
										});
									},
									fail(err) { // getBLEDeviceCharacteristics
										console.error(err);
										uni.showToast({title:'设备连接失败:3'});
									}
								})
						    },
						    fail(err) { // getBLEDeviceServices
						        console.error(err);
								uni.showToast({title:'设备连接失败:2'});
						    }
						})
			        },
			        fail(err) { // createBLEConnection
			            console.error(err);
