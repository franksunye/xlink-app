		...
		<mp-form :inline="false">
			<mp-cells>
				...
				<mp-cell title="F1快捷键" ext-class="cell-picker">
					<uni-data-picker :clear-icon="false" class="picker-single" placeholder="请选择快捷键" popup-title="请选择快捷键"
						:localdata="funcArray" v-model="funcF1">
					</uni-data-picker>
				</mp-cell>
				<mp-cell title="F2快捷键" ext-class="cell-picker">
					<uni-data-picker :clear-icon="false" class="picker-single" placeholder="请选择快捷键" popup-title="请选择快捷键"
						:localdata="funcArray" v-model="funcF2">
					</uni-data-picker>
				</mp-cell>
				<mp-cell title="F3快捷键" ext-class="cell-picker">
					<uni-data-picker :clear-icon="false" class="picker-single" placeholder="请选择快捷键" popup-title="请选择快捷键"
						:localdata="funcArray" v-model="funcF3">
					</uni-data-picker>
				</mp-cell>
			</mp-cells>
			<view class="buttons">
				<button @tap="doBtn('cancel')">取消</button>
				<button @tap="doBtn('ok')" style="background: #106CFF;color: #fff">确定</button>
...
<script>
	export default {
		...
		onReady() {
			if(uni.getStorageSync('distance'))
				this.distance = uni.getStorageSync('distance');
			if(uni.getStorageSync('inter'))
				this.inter = uni.getStorageSync('inter');
			if(uni.getStorageSync('deviation'))
				this.deviation = uni.getStorageSync('deviation');
			if(uni.getStorageSync('markDegree'))
				this.markDegree = uni.getStorageSync('markDegree');
			...
			if(uni.getStorageSync('markTxtSize'))
				this.markTxtSize = uni.getStorageSync('markTxtSize');
			if(uni.getStorageSync('startPointGap'))
				this.startPointGap = uni.getStorageSync('startPointGap');
			if(uni.getStorageSync('wheelRadius'))
				this.wheelRadius = uni.getStorageSync('wheelRadius');
			if(uni.getStorageSync('funcF1'))
				this.funcF1 = uni.getStorageSync('funcF1');
			if(uni.getStorageSync('funcF2'))
				this.funcF2 = uni.getStorageSync('funcF2');
			if(uni.getStorageSync('funcF3'))
				this.funcF3 = uni.getStorageSync('funcF3');
		},
		methods:{
			doBtn(id) {
				if(id == 'ok') {
