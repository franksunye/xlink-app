<template>
  <view class="purchaseOrderItems" @click.capture="track">
    <!-- 过滤 -->
      ...
      >
        <!-- <view slot="action" class="c-flex">
          <van-icon
            ...
            name="filter-o"
            @click="filterSortingShow = !filterSortingShow" :data-event="{id:'_t360UpC4g'}"></van-icon>
        </view> -->
      </van-search>
    </view>
    ...
    <view class="c-content">
      ...
      <view class="checkAll" v-if="!disBatchBtn">
        <van-checkbox
        ...
        <view style="margin: 20rpx" v-for="(newListItem, j) in newList" :key="j">
          <div class="c-card">
            ...
            <view class="c-card-content">
              <view class="c-flex">
                <text>创建人：{{ newListItem[0].createUserName || '' }}</text>
              </view>
              <view class="c-flex">
                <text>创建时间：{{ newListItem[0].createTime || '' }}</text>
              </view>
              <view class="c-flex">
              ...
              <view class="c-flex">
                <text>订单总价：¥ {{ newListItem[0].totalPrice || '' }}</text>
              </view>
              <!-- <view class="c-flex">
                <text>供应商名称：{{ newListItem[0].supplierName || '' }}</text>
              </view> -->
              <!-- <view
                class="c-flex"
                @click.native.stop="makeCall(newListItem[0] && newListItem[0].supplierPhone)" :data-event="{id:'VhQC1PYKDU'}">
                <text :data-event="{id:'VhQC1PYKDU'}">供应商电话：{{ newListItem[0].supplierPhone || '' }}</text>
                <van-icon name="phone-o" class="c-m-l-20" :data-event="{id:'VhQC1PYKDU'}" />
              </view> -->
              <!-- <view class="c-flex">
          ...
          <view class="c-card" v-for="(item, k) in newListItem" :key="k" :data-id="item._id">
            <view class="c-card-title" style="border-bottom: 0px solid #d4d4d4">
              <view class="c-flex">
                <div class="list-checkbox" v-if="item.status == '301'">
                  <van-checkbox
                    :value="item.checked"
                    @change="onCheck($event, item)"
                  ></van-checkbox>
                </div>
                <!-- <van-icon custom-class="c-card-icon" name="logistics"></van-icon> -->
                <text style="font-size: 13px">发货单状态</text>
              </view>
              <view :style="'font-size: 14px;font-weight: 600;'" class="c-red">
                {{
            ...
            <view style="display: flex">
              ...
              <view class="c-card-content">
                ...
                <view class="materialDetails">
                  <text>供应商：{{ item.supplierName || '' }}</text>
                </view>
                <view
                  class="materialDetails"
                  @click.native.stop="makeCall(item && item.supplierPhone)"
                  :data-event="{ id: 'YJgekuz4Fq' }"
                >
                  <text :data-event="{ id: 'YJgekuz4Fq' }">
                    供应商电话：{{ item.supplierPhone || '' }}
                  </text>
                  <van-icon
                    name="phone-o"
                    class="c-m-l-20"
                    v-if="item.supplierPhone"
                    :data-event="{ id: 'YJgekuz4Fq' }"
                  />
                </view>

                <view
                  class="c-flex materialDetails"
                  v-if="item.status == '499' || item.status == '498'"
                >
            ...
            <!-- currentRole_id == '2449148386138867771' 修链平台管理员 -->
            <view class="c-m-t-20">
              <view
                v-if="item.status !== '499' && item.status !== '498'"
              ...
              >
                <van-stepper
                  ...
                  disabled
                />
                <text class="c-red c-bold c-m-l-20" v-if="item.price || item.price === 0">
                  ¥{{ item.price }}
                </text>
              ...
              <view class="c-flex c-flex-end">
                ...
                <!-- <view class="dispatchBtn" v-if="item.status == '101'">
                  <van-button round color="#A9A9A9" size="small" @click="doOperate(item, 101, newListItem)" :data-event="{id:'1h8ykzQgJ5'}">
                    撤回
                  </van-button>
                </view> -->
                ...
                <view
                  class="dispatchBtn"
                  v-if="item.status == '101' && currentRole_id == '2449148386138867771'"
                >
                  <van-button
                    plain
                    round
                ...
                <view
                  class="dispatchBtn"
                  v-if="item.status == '102' && currentRole_id == '2449148386138867771'"
                >
                  <van-button
                    plain
                    round
                ...
                <view
                  class="dispatchBtn"
                  v-if="item.status == '201' && currentRole_id == '2449148386138867771'"
                >
                  <van-button
                    plain
                    round
                ...
                <view class="dispatchBtn" v-if="item.status == '301'">
                  <van-button
                    ...
                    round
                    size="small"
                    @click="openSignFor(item, 401)"
                    :data-event="{ id: 'jvxSj-IKge' }"
                  >
                    签收
                  </van-button>
                </view>
                <view class="dispatchBtn" v-if="item.status == '401' && item.images">
                  <van-button
                    ...
                    size="small"
                    @click="openView(item)"
                    :data-event="{ id: 'qUavoa6xmU' }"
                  >
                    查看
                  </van-button>
                </view>
                <view class="dispatchBtn" v-if="item.status == '401'">
                  <van-button
                    round
                    type="danger"
                    size="small"
                    @click="openReturnedPurchase(item)"
                    :data-event="{ id: 'KiW76c_g-a' }"
                  >
                    退货
                  </van-button>
                </view>
...
export default {
  data() {
    return {
      ...
      allRowList: [], //勾选上的全部数据
      rowList: [], //勾选上的dan
      // currentType: '', //缓存中角色的type
      selectAll: false,
      purchaseOrderItemStatus: '', //发货单状态
      newList: [], //按照创建时间排序后的数据
      currentRole_id: '', //当前登陆人的ID  currentRole_id == '2449148386138867771' 伙伴服务商平台管理员ID
      //签收上传图片
      signFor: {
        show: false,
        list: []
      ...
      disBatchBtn: false //隐藏批量操作按钮
      // signForShow:false  //签收上传图片signForList
    };
  },
  methods: {
    onLoad: function (option) {
      let _currentUser = wx.getStorageSync('currentRole');
      // this.currentType = _currentUser.type;
      this.currentRole_id = _currentUser._id;
      this.purchaseOrderId = option.purchaseOrderId;
      this.purchaseOrderItemStatus = option.status;
      if (option.status == 401) {
        this.disBatchBtn = true;
      } else {
        this.disBatchBtn = false;
      }
      this.doInit();
    },
    doInit(param) {
    ...
    // 搜索供应商
    supplierSearchValue(e) {
      this.supplier.searchValue = e.detail;
      this.getSupplier();
    },
    // 选择供应商
    saveSupplier() {
      let arr = [];
      arr.push(this.supplier.list);
      var params = {
        purchaseOrderItemList: arr,
        supplierId: this.supplier.selectId,
        newStatus: '102'
      };
      console.log(params, 'params');
      common.osg.ajax(
      ...
      this.supplier.list = [];
      this.supplier.selectId = '';

      this.supplier.show = false;
    },
    // 获取已选中供应商的id
    supplierChange(e) {
      this.supplier.selectId = e.detail;
    },
    // 获取供应商
    getSupplier() {
      var params = {
        ...
        sortOrder: 'desc',
        notFilterOrgId: 1,
        'regex:title|string#or': this.supplier.searchValue,
        'regex:name|string#or': this.supplier.searchValue,
        'regex:photo|string#or': this.supplier.searchValue,
        'regex:address|string#or': this.supplier.searchValue,
        'regex:describe|string#or': this.supplier.searchValue,
        'regex:orderNum|string#or': this.supplier.searchValue
      };
      common.osg.ajax('basic/supplier/query.do', params, res => {
