<template>
  <view class="partnerCreditItem" @click.capture="track">
    ...
    <view class="creditItem" v-for="(item, index) in data" :key="index">
      <view>
        <view>
          <text v-if="item.status == 0" class="c-blue">待审核</text>
          <text v-else-if="item.status == 1" class="c-green">审核通过</text>
          <text v-else-if="item.status == -1" class="c-red">审核未通过</text>
        </view>
        <view class="createTime">{{ item.createTime }}</view>
        <view v-if="item.status == -1 && item.failMemo" class="createTime c-m-r-20">
          原因：{{ item.failMemo }}
        </view>
      </view>
      <view class="amount c-orange">
        <text class="c-size-20">+</text>
        <text class="c-size-22 c-m-l-10">{{ item.payAmount }}</text>
      </view>
    </view>
...
export default {
  ...
  onLoad(params) {
    ...
    this.getRechargeForm();
  },
  methods: {
    getRechargeForm() {
      common.osg.ajax(
        '/basic/rechargeForm/query',
        {
          _all: '1',
          sortField: 'createTime',
          sortOrder: 'desc',
          'is:state|integer#and': 1,
          'gte:createTime|datetime#and': this.formatStartDate(),
          'lte:createTime|datetime#and': this.formatEndDate()
        },
        res => {
          this.data = res.data;
        }
