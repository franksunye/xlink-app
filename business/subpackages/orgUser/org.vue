<template>
  <view class="orgUsers" @click.capture="track">
    <view>
      ...
      <view class="selectedOrg-view">
        <block v-for="(item, i) in selectedOrgs" :key="i">
          ...
          <block v-else>
            <text class="c-blue" @click="selectedOrgClick(i)" :data-event="{ id: '78STu2ql8w' }">
              {{ item.orgName }}
            </text>
            <van-icon name="arrow" />
          </block>
    ...
    <view class="orgUser">
      ...
      <!-- 当前组织用户 -->
      <view class="c-m-t-20 c-bg-white">
        <view
          v-for="(item, i) in users"
          :key="i"
          class="user-view"
          @click="userClick(item)"
          :data-event="{ id: 'MZPhoZDUsH' }"
        >
          <van-image
            width="60"
            height="60"
            ...
            :data-event="{ id: 'MZPhoZDUsH' }"
          />
          <view class="user-info" :data-event="{ id: 'MZPhoZDUsH' }">
            <view :data-event="{ id: 'MZPhoZDUsH' }">
              <text class="c-bold c-m-r-20" :data-event="{ id: 'MZPhoZDUsH' }">
                {{ item.name }}
              </text>
              <text class="c-color-value" :data-event="{ id: 'MZPhoZDUsH' }">{{ item.phone }}</text>
            </view>
            <view :data-event="{ id: 'MZPhoZDUsH' }">
              <van-tag
    ...
    <view class="orgUser-add">
      ...
      <view @click="doOpen('./user')" :data-event="{ id: 'ad1K_rA3cq' }">
        <van-icon name="add" class="c-blue" :data-event="{ id: 'ad1K_rA3cq' }" />
        <text :data-event="{ id: 'ad1K_rA3cq' }">添加用户</text>
      </view>
    </view>
  </view>
</template>
    ...
    getUsers() {
      common.osg.ajax(
        'user/query',
        {
          ...
          'is:orgIds|array#and': this.selectedOrgs[this.selectedOrgs.length - 1]._id,
