<template>
  <view class="c-box" :class="openForm ? 'c-hidden c-h-50rpx' : ''" @click.capture="track">
    <!-- 摘要信息 -->
    <view class="c-abstract">
      <view class="c-flex-between">
        ...
        <view class="c-orange">
          {{ codelabel('select', 'workOrderStatus', SAFormData.status, opts.status) || '' }}
        </view>
      </view>
      <!-- <view>
        <text>
      ...
      <view>
        <text>售后工单编号：{{ SAFormData.code || '' }}</text>
      </view>
      <view v-if="SAFormData.exts.serviceAppointmentCode">
        <text>服务工单编号：</text>
        <text
          class="c-blue"
          @click="
            open('/subpackages/serviceAppointment/serviceAppointment', {
              id: SAFormData.serviceAppointmentId
            })
          "
          :data-event="{ id: 'TuyFdRDb9c' }"
        >
          {{ SAFormData.exts.serviceAppointmentCode || '' }} 点击跳转
        </text>
      </view>
      <view>
    ...
    <view>
      <van-tabs
        ...
        :ellipsis="false"
        ref="tabs"
        :data-event="{ id: 'tddA_AbWSi', capture: false }"
      >
                ...
                >
                  ...
                  <block v-else-if="extField.type == 'file'">
                    <view
                      ...
                      :data-event="{ id: 'tddA_AbWSi' }"
                    >
                      <view class="c-cell_title" :data-event="{ id: 'tddA_AbWSi' }">
                        {{ extField.nameDisp }}
                      </view>
                      <view class="c-cell_value" :data-event="{ id: 'tddA_AbWSi' }">
                        （{{
                          extField.preset
                            ? SAFormData[extField.name].length || 0
                    ...
                    <view class="c-cell-view" :data-event="{ id: 'tddA_AbWSi' }">
                      <view
                      ...
                      >
                        <video
                          ...
                          @click="openVideo"
                          :data-event="{ id: 'NNde9K57vV', capture: false }"
                        ></video>
                      </view>
                      <van-uploader
                        style="display: flex"
                  ...
                  <block v-else-if="extField.type == 'gap'">
                    <view class="c-cell-gap" :data-event="{ id: 'tddA_AbWSi' }">
                      {{ (extField.frontEndConfig && extField.frontEndConfig.html) || '' }}
                    </view>
                  </block>
                  ...
                  <block v-else-if="extField.type == 'scan'">
                    <view
                      ...
                      :data-event="{ id: 'tddA_AbWSi' }"
                    >
                      <text class="c-cell_title" :data-event="{ id: 'tddA_AbWSi' }">
                        {{ extField.nameDisp }}
                      </text>
                      <van-button
                        disabled
                  ...
                  <block v-else-if="extField.type == 'addItem'">
                    <view
                      ...
                      :data-event="{ id: 'tddA_AbWSi' }"
                    >
                      <text :data-event="{ id: 'tddA_AbWSi' }">{{ extField.nameDisp }}</text>
                      <van-button
                        disabled
                  ...
                  <block v-else-if="extField.type == 'sort'">
                    ...
                    <van-cell-group :border="false">
                      ...
                      >
                        ...
                        <view :data-event="{ id: 'tddA_AbWSi' }">
                          <text :data-event="{ id: 'tddA_AbWSi' }">{{ item.length }}</text>
                          <text :data-event="{ id: 'tddA_AbWSi' }">
        ...
        <van-tab title="处理记录" name="note" style="margin-bottom: 75px">
          <view
            class="c-card"
            ...
            :data-event="{ id: 'tddA_AbWSi' }"
          >
            <view class="c-card-title" :data-event="{ id: 'tddA_AbWSi' }">
              <view class="c-flex c-flex-item" :data-event="{ id: 'tddA_AbWSi' }">
                <van-icon
                  custom-class="c-card-icon"
                  name="todo-list-o"
                  :data-event="{ id: 'tddA_AbWSi' }"
                ></van-icon>
                <text :data-event="{ id: 'tddA_AbWSi' }">{{ item.createTime }}</text>
              </view>
            </view>
            <view class="c-p-20" style="overflow: auto" :data-event="{ id: 'tddA_AbWSi' }">
              <rich-text :nodes="item.content"></rich-text>
            </view>
            <view class="c-p-v-20 c-flex c-flex-between" :data-event="{ id: 'tddA_AbWSi' }">
              <view :data-event="{ id: 'tddA_AbWSi' }">操作人：{{ item.createUserName }}</view>
            </view>
          </view>
        </van-tab>
        <!-- 渲染工作流节点 -->
          ...
          >
            ...
            >
              <van-tabs
                ...
                :data-event="{ id: 't40GZmdCVo', capture: false }"
              >
                <van-tab title="订单" name="order">
                  <view
                    ...
                    :data-event="{ id: '_BID1ZPvvJ' }"
                  >
                    <view class="c-card-title" :data-event="{ id: '_BID1ZPvvJ' }">
                      <view class="c-flex c-flex-between c-w" :data-event="{ id: '_BID1ZPvvJ' }">
                        <view :data-event="{ id: '_BID1ZPvvJ' }">
                          <van-icon
                            custom-class="c-card-icon"
                            name="balance-list-o"
                            :data-event="{ id: '_BID1ZPvvJ' }"
                          ></van-icon>
                          <text :data-event="{ id: '_BID1ZPvvJ' }">{{ item._id }}</text>
                        </view>
                        <view class="c-orange" :data-event="{ id: '_BID1ZPvvJ' }">
                          <text :data-event="{ id: '_BID1ZPvvJ' }">
                            {{ item.state == '1' ? '有效' : '无效' }}
                          </text>
                        </view>
                      </view>
                    ...
                    <view :data-event="{ id: '_BID1ZPvvJ' }">
                      <text :data-event="{ id: '_BID1ZPvvJ' }">
                        类型：
                        <text class="c-bold" :data-event="{ id: '_BID1ZPvvJ' }">
                          {{ item.type == '1' ? '订单' : '报价单' }}
                        </text>
                      </text>
                      <text
                    ...
                    >
                      <text :data-event="{ id: '_BID1ZPvvJ' }">
                        <!-- class="c-red" -->
                        <text :data-event="{ id: '_BID1ZPvvJ' }">折扣金额：</text>
                        <text :data-event="{ id: '_BID1ZPvvJ' }">
                          - ¥{{
                            amountToFixed(
                              item.originPrice - amountToFixed(item.originPrice * item.discount)
                    ...
                    <view v-if="item.preferentialAmount" :data-event="{ id: '_BID1ZPvvJ' }">
                      <text :data-event="{ id: '_BID1ZPvvJ' }">
                        优惠金额：
                        <!-- class="c-red" -->
                        <text :data-event="{ id: '_BID1ZPvvJ' }">
                    ...
                    <view :data-event="{ id: '_BID1ZPvvJ' }">
                      <text :data-event="{ id: '_BID1ZPvvJ' }">
                        实际金额：
                        <text class="c-red" :data-event="{ id: '_BID1ZPvvJ' }">
                          ¥ {{ item.totalPrice }}
                        </text>
                      </text>
                    </view>
                    ...
                    <view style="justify-content: flex-end" :data-event="{ id: '_BID1ZPvvJ' }">
                      <view :data-event="{ id: '_BID1ZPvvJ' }"></view>
                      <van-button
                        round
                        type="info"
                        size="mini"
                        plain
                        @click.native.stop="pay(item)"
                        v-if="
                ...
                <van-tab title="合同" name="contract">
                  <view
                    ...
                    :key="i"
                    :data-event="{ id: 't40GZmdCVo' }"
                  >
                    <view class="c-card-title" :data-event="{ id: 't40GZmdCVo' }">
                      <view
                        class="c-flex c-flex-between c-flex-center c-w"
                        :data-event="{ id: 't40GZmdCVo' }"
                      >
                        <view class="c-flex c-flex-center" :data-event="{ id: 't40GZmdCVo' }">
                          <van-icon
                            custom-class="c-card-icon"
                            name="description"
                            :data-event="{ id: 't40GZmdCVo' }"
                          ></van-icon>
                          <text :data-event="{ id: 't40GZmdCVo' }">{{ item.contractdocNum }}</text>
                        </view>
                        <text class="c-orange" :data-event="{ id: 't40GZmdCVo' }">
                          {{
                            commonEvent('codelabel', null, {
                    ...
                    <view :data-event="{ id: 't40GZmdCVo' }">
                      <text :data-event="{ id: 't40GZmdCVo' }">
                        委托方：{{ item.contactsName || item.client || '' }}
                      </text>
                    </view>
                    <view :data-event="{ id: 't40GZmdCVo' }">
                      <text :data-event="{ id: 't40GZmdCVo' }">
                    ...
                    <view :data-event="{ id: 't40GZmdCVo' }">
                      <text :data-event="{ id: 't40GZmdCVo' }">
                        签约时间：{{ item.signedDate || '' }}
                      </text>
                    </view>
                    <view class="c-m-t-20" :data-event="{ id: 't40GZmdCVo' }">
                      <van-button
                        round
                        ...
                        "
                        :data-event="{ id: 'DXetseMREI' }"
                      >
                        取消合同
                      </van-button>
                      <view v-else style="width: 160rpx" :data-event="{ id: 't40GZmdCVo' }"></view>
                      <van-button
                        round
                        @click="signAgain(item._id)"
                        v-if="item.exts.contractStatus == '-3'"
                        :data-event="{ id: 'nYVy56jqiI' }"
                      >
                        再次签约
                      </van-button>
                      <van-button
                      ...
                      <van-button
                        ...
                        :data-event="{ id: 'Dfqpek113H' }"
                      >
                        查看文件
                      </van-button>
                      <view v-else :data-event="{ id: 't40GZmdCVo' }"></view>
                    </view>
                  </view>
                </van-tab>
              </van-tabs>
            ...
            <van-tab
              ...
              :name="node"
            >
              <view style="padding-bottom: 140rpx" :data-event="{ id: 'tddA_AbWSi' }">
                <block v-for="(subflow, index1) in subflows" :key="index1">
                  ...
                  >
                    <view
                      ...
                      :data-event="{ id: 'tddA_AbWSi' }"
                    >
                      <view class="c-flex" :data-event="{ id: 'tddA_AbWSi' }">
                        <van-icon
                          custom-class="c-card-icon"
                          name="balance-list-o"
                          :data-event="{ id: 'tddA_AbWSi' }"
                        ></van-icon>
                        <text class="c-nowrap" :data-event="{ id: 'tddA_AbWSi' }">
                          {{ subflow.title }}
                        </text>
                      </view>
                      <view class="c-orange c-nowrap" :data-event="{ id: 'tddA_AbWSi' }">
                        {{
                    ...
                    <view class="c-card-content" :data-event="{ id: 'tddA_AbWSi' }">
                      <view :data-event="{ id: 'tddA_AbWSi' }">
                        添加时间：{{ subflow.createTime }}
                      </view>
                    </view>
                    <view style="min-height: 60rpx" :data-event="{ id: 'tddA_AbWSi' }">
                      <view :data-event="{ id: 'tddA_AbWSi' }"></view>
                      <van-button
                ...
                >
                  ...
                  >
                    ...
                    <view class="c-orange c-nowrap" :data-event="{ id: 'tddA_AbWSi' }">
                      ...
                      <text v-else :data-event="{ id: 'tddA_AbWSi' }">已完成</text>
                    </view>
                  </view>
                  <view class="c-card-content" :data-event="{ id: 'tddA_AbWSi' }">
                    <view :data-event="{ id: 'tddA_AbWSi' }">创建时间：{{ form.createTime }}</view>
                  </view>
                  <view style="min-height: 60rpx" :data-event="{ id: 'tddA_AbWSi' }">
                    <van-button
                      v-if="form.state != -1"
                      ...
                      round
                      :data-event="{ id: 'S00wmYtFFZ' }"
                    >
                      删除
                    </van-button>
                    <view v-else :data-event="{ id: 'tddA_AbWSi' }"></view>
                    <van-button
                      @click="viewAddForm(form.formDefId, form.formTitle, form._id, form)"
                      round
                      type="info"
                      size="mini"
                      :data-event="{ id: 'qg-Ju869eT' }"
                    >
                      查看
                    </van-button>
                  </view>
              ...
              <view style="padding-bottom: 140rpx" :data-event="{ id: 'tddA_AbWSi' }">
                ...
                >
                  ...
                  >
                    ...
                    <!-- <view class="c-orange c-nowrap" :data-event="{id:'tddA_AbWSi'}">
                      <text v-if="form.state == -1" class="c-red" :data-event="{id:'tddA_AbWSi'}">已删除</text>
                      <text v-else :data-event="{id:'tddA_AbWSi'}">已完成</text>
                    </view> -->
                    <view class="c-orange c-nowrap" :data-event="{ id: 'tddA_AbWSi' }">
                      <text
                        v-if="currentNode.state == 10 || form.nodeId == currentNode._id"
                        :data-event="{ id: 'tddA_AbWSi' }"
                      >
                        已完成
                      </text>
                      <text v-else :data-event="{ id: 'tddA_AbWSi' }">待填写</text>
                    </view>
                  </view>
                  <view class="c-flex c-flex-center" :data-event="{ id: 'tddA_AbWSi' }">
                    <view class="c-card-content" :data-event="{ id: 'tddA_AbWSi' }">
                      <view :data-event="{ id: 'tddA_AbWSi' }">
                  ...
                  <!-- <view style="min-height: 60rpx" :data-event="{id:'tddA_AbWSi'}">
                    <van-button
                      v-if="form.state != -1"
                      @click="removeForm(form)"
                      round :data-event="{id:'KCMH5eIDpo'}">
                      删除
                    </van-button>
                    <view v-else :data-event="{id:'tddA_AbWSi'}"></view>
                    <view :data-event="{id:'tddA_AbWSi'}"></view>
                    <van-button
                      ...
                      round
                      type="info"
                      size="mini" :data-event="{id:'x064mFDJf8'}">
                      查看
                    </van-button>
                  </view> -->
                </view>
                        ...
                        <view class="c-cell-view" :data-event="{ id: 'tddA_AbWSi' }">
                          ...
                          >
                            <video
                              ...
                              :show-fullscreen-btn="false"
                              @click="openVideo"
                              :data-event="{ id: 'Dszi1E-Cml', capture: false }"
                            ></video>
                          </view>
                          <van-uploader
                            style="display: flex"
              ...
              <view class="c-card-title" :data-event="{ id: 'tddA_AbWSi' }">
                <view class="c-flex" :data-event="{ id: 'tddA_AbWSi' }">
                  <van-icon
                    ...
                    :data-event="{ id: 'tddA_AbWSi' }"
                  ></van-icon>
                  <text :data-event="{ id: 'tddA_AbWSi' }">{{ item.title }}</text>
                </view>
              </view>
              <view class="c-p-20" :data-event="{ id: 'tddA_AbWSi' }">
                <text :data-event="{ id: 'tddA_AbWSi' }">{{ item.content }}</text>
              </view>
              <view class="c-p-v-20 c-flex c-flex-between" :data-event="{ id: 'tddA_AbWSi' }">
                <view :data-event="{ id: 'tddA_AbWSi' }">
                  操作人：{{ item.createUserName || '系统' }}
                </view>
                <view class="c-size-14" :data-event="{ id: 'tddA_AbWSi' }">
                  {{ item.createTime }}
                </view>
              </view>
            </view>
    ...
    <view class="c-button-group-bottom" v-if="activeTab.detail.name == 'info'">
      <view
        ...
        :data-event="{ id: '5egCM6KrCn' }"
      >
        <view class="c-white" :data-event="{ id: '5egCM6KrCn' }">打电话</view>
        <view class="c-size-14" :data-event="{ id: '5egCM6KrCn' }">联系客户</view>
      </view>
      <view
        v-else
        @click="
        ...
        "
        class="c-button-group-item c-m-r-10"
        :data-event="{ id: 'wBVH5mfqVF' }"
      >
        <view class="c-white" :data-event="{ id: 'wBVH5mfqVF' }">打电话</view>
        <view class="c-size-14" :data-event="{ id: 'wBVH5mfqVF' }">联系客户</view>
      </view>
      <!-- <view @click="doEdit" class="c-button-group-item c-m-l-10" :data-event="{id:'dfpCEm0xMB'}">
        <view class="c-white" :data-event="{id:'dfpCEm0xMB'}">填信息</view>
        <view class="c-size-14" :data-event="{id:'dfpCEm0xMB'}">填写工单信息</view>
      </view> -->
      <!-- <view
        v-if="changeStatusAuth"
        @click="statusClick"
        class="c-button-group-item c-m-l-10" :data-event="{id:'ox28LASnSe'}">
        <view class="c-white" :data-event="{id:'ox28LASnSe'}">{{ changeStatusButton }}</view>
        <view class="c-size-14" :data-event="{id:'ox28LASnSe'}"></view>
      </view> -->
    </view>
    ...
    >
      ...
      <view
        ...
        :data-event="{ id: '4PFhzAaY31' }"
      >
        <view :data-event="{ id: '4PFhzAaY31' }">修改</view>
        <!-- <view class="c-size-14" :data-event="{id:'4PFhzAaY31'}"></view> -->
      </view>
      <!-- 回退 -->
      <view
        v-if="rollbackShow"
        ...
        class="c-button-group-item c-m-r-10 c-bg-gray-button"
        :data-event="{ id: 'T-_m3eUzI1' }"
      >
        <view :data-event="{ id: 'T-_m3eUzI1' }">删除{{ activeTab.detail.name.nodeDef.name }}</view>
        <!-- <view class="c-size-14" :data-event="{id:'T-_m3eUzI1'}"></view> -->
      </view>
      <!-- 编辑表单 -->
      <view
        v-if="
        ...
        :data-event="{ id: 'qWvQDDir6k' }"
      >
        <!-- 修改填表单 -->
        <view class="c-white" :data-event="{ id: 'qWvQDDir6k' }">处理完成</view>
        <!-- <view class="c-size-14" :data-event="{id:'qWvQDDir6k'}">{{fabShow?'填写表单信息':'已完成不允许修改'}}</view> -->
        <van-icon
          v-if="!fabShow"
      ...
      <view
        ...
        @click="addForm"
        class="c-button-group-item c-m-l-10 c-m-r-10"
        :data-event="{ id: '4DZ6BQTNvC' }"
      >
        <view class="c-white" :data-event="{ id: '4DZ6BQTNvC' }">
          新增{{ activeTab.detail.name.nodeDef.name }}
        </view>
      </view>
      <!-- 新增表单 -->
      <view
        ...
        :data-event="{ id: 'URBaPt2GGS' }"
      >
        <view class="c-white" :data-event="{ id: 'URBaPt2GGS' }">新增表单</view>
        <van-icon
      ...
      <view
        ...
        :class="activeTab.detail.name.state != 10 && repeatComplateEnable ? '' : 'c-no-pointer'"
        class="c-button-group-item c-m-l-10 c-bg-green"
        :data-event="{ id: 'IldEf2Ppny' }"
      >
        <view class="c-white" :data-event="{ id: 'IldEf2Ppny' }">
          完成{{ activeTab.detail.name.nodeDef.name }}
        </view>
        <!-- <view class="c-size-12" :data-event="{id:'IldEf2Ppny'}"></view> -->
        <van-icon
          v-if="activeTab.detail.name.state == 10 || !repeatComplateEnable"
          class="c-icon-disable"
          name="https://pub.fsgo365.cn/7527232764334981828.png"
          :data-event="{ id: 'IldEf2Ppny' }"
        ></van-icon>
      ...
      <view
        ...
        @click="placeOrder"
        class="c-button-group-item c-m-l-10 c-m-r-10"
        :data-event="{ id: 'dvMj0PXjSr' }"
      >
        <view class="c-white" :data-event="{ id: 'dvMj0PXjSr' }">下单</view>
      </view>
      <!-- 新增服务项 -->
      <view
        v-if="
        ...
        :class="activeTab.detail.name.state != 10 ? '' : 'c-no-pointer'"
        class="c-button-group-item c-m-l-10 c-m-r-10"
        :data-event="{ id: 'OqRZO_bau0' }"
      >
        <view class="c-white" :data-event="{ id: 'OqRZO_bau0' }">新增服务</view>
      </view>
      <!-- 完成该节点 -->
      <view
        ...
        class="c-button-group-item c-m-l-10 c-bg-green"
        :data-event="{ id: 'tMZ9slDESg' }"
      >
        <view class="c-white" :data-event="{ id: 'tMZ9slDESg' }">
          完成{{ activeTab.detail.name.nodeDef.name }}
        </view>
        <!-- <view class="c-size-12" :data-event="{id:'tMZ9slDESg'}"></view> -->
        <van-icon
    ...
    <view class="c-button-group-bottom" v-if="activeTab.detail.name == 'note'">
      <view
        ...
        "
        class="c-button-group-item c-m-r-10"
        :data-event="{ id: 'a-zJITGOan' }"
      >
        <view class="c-white" :data-event="{ id: 'a-zJITGOan' }">填写处理记录</view>
      </view>
    </view>
          ...
          <view class="c-card" style="margin: 20rpx 0">
            ...
            <view style="min-height: 60rpx">
              <view v-if="subNode.visible">
                <van-button
                  custom-style="min-width: 140rpx;height:80rpx;background-color:rgb(239,239,239);"
                  round
                  type="default"
                  size="normal"
                  v-if="subNode.state == 10 && subWorkflow.def.nodes[index2 + 1].state == 0"
                  @click="takebackWorkflow(subNode._id)"
                  :data-event="{ id: '8v_WjIVLnM' }"
                >
                  修改
                </van-button>
                <van-button
                  ...
                  <view class="c-cell-view">
                    <view v-if="extField.frontEndConfig.fileType == 'video'" class="c-flex">
                      <video
                        ...
                        :show-fullscreen-btn="false"
                        @click="openVideo"
                        :data-event="{ id: 'gaejAP06CM', capture: false }"
                      ></video>
                    </view>
                    <van-uploader
                      style="display: flex"
      ...
      <view class="quicklyFill-wrapper" v-if="isQuicklyFill">
        <view class="quicklyFill-content">
          <!-- <van-button size="small" :class="groupActionNum[currentNode._id]>1 && isQuicklyFill?'':'hidden'" color="#545c64"
					 plain @click="fillPrevious" :data-event="{id:'jQXAFXbbJw'}">上一步</van-button>
					<view :class="groupActionNum[currentNode._id]>1 && isQuicklyFill?'hidden':''"></view>
					<van-button custom-style="width: 100%;min-height: 110rpx;"  v-if="isQuicklyFill" custom-class="c-button-group-item c-m-l-10" color="#545c64" @click="fillNext" :data-event="{id:'qnlJ8Dq2aA'}">{{groupActionNum[currentNode._id]==groupLength?'提交':'下一步'}}</van-button>
          <view></view>-->
          <view
            :class="
              groupActionNum[currentNode._id] > 1 && isQuicklyFill
                ? 'c-button-group-item c-m-r-10 c-bg-gray-button'
                : 'hidden'
            "
            color="#545c64"
            plain
            @click="fillPrevious"
            :data-event="{ id: '447t52w2Ue' }"
          >
            上一步
          </view>
          <view
          ...
          <view
            ...
            color="#545c64"
            @click="fillNext"
            :data-event="{ id: 'YlbWwR2p9v' }"
          >
            <view
              :class="disabled.saveForm ? 'c-no-pointer' : ''"
              :data-event="{ id: 'YlbWwR2p9v' }"
            >
    ...
    >
      <view class="c-flex c-flex-col c-flex-center">
        <view style="padding: 30rpx 0 30rpx">微信/支付宝收款</view>
        <!-- <view style="padding-bottom: 30rpx;">请向客户出示收款码，客户扫码后即可收款。</view> -->
        <view
          v-if="payment.state.paid >= payment.state.total"
        ...
        <view style="padding-top: 30rpx">
          <view class="c-p-10" v-if="payment.state.msg1">{{ payment.state.msg1 }}</view>
          <view class="c-p-10" v-if="payment.state.msg2">{{ payment.state.msg2 }}</view>
          <view class="c-p-10" v-if="payment.state.msg2">{{ payment.state.msg3 }}</view>
    ...
    <van-dialog
      ...
      @cancel="dialogCancel"
      :data-event="{ id: '_xOWSztwLH', capture: false }"
    >
    ...
    <van-dialog
      ...
      width="100vw"
      @confirm="closeVideo"
      :data-event="{ id: 'E_jB1LPzrs', capture: false }"
    >
      <video
        :src="video.url"
        controls
    ...
    <!-- 	<van-dialog
	  ...
	  asyncClose
	  show-cancel-button     
	  @confirm="cancelContractAjax" :data-event="{id:'EJixAWfC_6',capture:false}">
	  <van-cell-group>
	     <van-field
  ...
  data() {
    return {
      ...
      payment: {
        open: false,
        cid: '',
        qrval: 'http://dev.fsgo365.cn',
        state: {}, //定时请求订单支付状态
        timer: null,
        success: false //是否全部支付
      },
  ...
  methods: {
    ...
    getSAData(refresh) {
      common.osg.ajax(
        ...
        { id: this.id },
        res => {
          this.SAFormData = res.data.workOrder;
          if (!refresh) {
          ...
          this.$nextTick(() => {
            ...
            let nodesTabs = this.nodes.filter(
              (node, index) =>
                ...
                node.nodeDef.nodeTypeOld != 'dispatch' &&
                index != 0 &&
                ((node.handlerIds && node.handlerIds.indexOf(this.userId) != -1) || node.visible)
            );
            //默认跳转最新节点
            this.vantTabsOntap(nodesTabs.length + 1);
            this.$forceUpdate();
          });
        },
        {
    ...
    getFields(callback) {
      common.osg.ajax(
        'biz/ext/queryByColName/workOrder',
        {
          sortField: 'order',
          sortOrder: 'desc',
        ...
        res => {
          ...
          common.osg.codeoption(() => {
            if (!this.opts.status) {
              this.opts.status = this.options.workOrderStatus;
            }
    ...
    doSave(type) {
      ...
      common.osg.ajax(
        'basic/workOrder/upsert.do',
        {
          workOrder: JSON.stringify(this.SAFormData)
        },
        res => {
          this.disabled.saveForm = false; //取消禁用提交表单按钮
          this.getSAData();
    ...
    closeSubWorkflow() {
      ...
      common.osg.ajax(
        ...
        { id: this.id },
        res => {
          this.SAFormData.status = res.data.workOrder.status;
        },
        {
          noload: true
        }
    ...
    getOrders(noload) {
      ...
      common.osg.ajax(
        ...
        res => {
          ...
          isReload = currentPage.route == 'subpackages/workOrder/workOrder' ? true : false;
    ...
    turnOrder(row) {
      if (this.orderTimer && window) window.clearTimeout(this.orderTimer);
      // this.getTurnOrderFields(formExtFields => {
      // 	common.osg.codeoption(() => {
      // 		// console.log(this.options);
      // 		this.openForm = true;
      // 		this.order.active = true;
      // 		this.extFields = formExtFields;
      // 		this.order.row = row;
      // 		this.formData.chooseOwnerType = '0'
      // 		this.extFields = this.extFields.filter((item) => (item.frontEndConfig && (item.frontEndConfig
      // 			.controller ||
      // 			item.frontEndConfig.isQuotation && item.frontEndConfig.chooseOwnerType == 2 || item.frontEndConfig.chooseOwnerType ==
      // 			0)))
      // 	});
      // });
      ...
      common.osg.ajax(
        ...
        res => {
          ...
          if (res.data.paid >= res.data.total) {
            this.payment.state = res.data; //更新支付状态
            clearInterval(this.payment.timer);
          } else {
            // 如果已付金额发生变化 则刷新收款码
            if (res.data.paid != this.payment.state.paid) {
              this.payment.qrval =
                common.osg.host +
                '/fsgo/i/pay/entry.do?state=' +
                id +
    ...
    getContracts(noload) {
      common.osg.ajax(
        ...
        {
          _all: '1',
          'is:workOrderId|string#and': this.id,
          'in:state|array-integer#and': '1',
            ...
            if (
              ...
              if (this.placeOrderPage.active == 'contract') {
                let contractTimer = setTimeout(() => {
                  ...
                  if (window && contractTimer) window.clearTimeout(contractTimer);
                }, 5000);
              }
            }
          }
    ...
    /* 去签约 */
    sign(item) {
      if (common.osg.isRepeatClick('sign')) {
        return; //判断是否重复点击
      }
      common.osg.confirm(
    ...
    open(url, data, options) {
      common.osg.open(url, data, options);
    }
  }
};
</script>
