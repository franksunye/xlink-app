  ...
  <view>
    ...
    <view class="c-content">
      <view>
        ...
        > -->
        <view
          style="margin: 10px 15px; background: #fff"
          ...
          <view style="display: flex; justify-content: space-around; padding: 10px">
            ...
            >
              <view>
                ...
                <van-tag style="float: right" color="#1989fa" size="medium">
              ...
              <view>
                <text>
                  省市区： {{ areaList.province_list[item.province || '']
                  }}{{ areaList.city_list[item.city] || ''
                  }}{{ areaList.county_list[item.district] || '' }}
                </text>
              </view>
              <view>
                <text>地址：{{ item.address || '' }}</text>
              </view>
              <view class="c-m-t-20" style="float: right; margin: 0">
                <!-- <image
                    style="width: 30px"
    ...
    <van-dialog use-slot :title="content" :show="tkiQrcodeShow" @confirm="closeQrcode">
      <view style="display: flex; justify-content: center; padding: 10px">
        <image mode="widthFix" style="width: 200px" :src="savepicRoute" />
      </view>
      <tki-qrcode
        v-if="tkiQrcodeShow"
        class="tki-qrcode"
...
export default {
  ...
  data() {
    return {
      ...
      picShow: false,
      userId: '',
      tkiQrcodeShow: false, //生成二维码
      chooseBtn: '' //选择生成二维码或海报
    };
  },
  methods: {
    ...
    getData() {
      var params = {
        ...
        // 'in:phone|array#or': this.searchValue,
        sortField: 'createTime',
        sortOrder: 'desc'
        // 'is:state|integer#and': 1
      };
      common.osg.ajax('wmt/biz/activity/query.do', params, res => {
        res.data.map(item => {
    ...
    doQrPosterQrcodeRoute(e) {
      // this.filePic(e, res => {
      this.savepicRoute = e;
      //   console.log(this.qrcodeRoute, '---qr二维码地址');
      // });
    },
    //分享给朋友
    share() {
