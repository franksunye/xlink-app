      ...
      <view class="outerCard" v-for="(data, k) in dataList" :key="k">
        <view>
          <view class="c-card-title">
            ...
            <!-- <view class="c-flex">工时：{{ data.workTime }}分钟</view> -->
          </view>
        ...
        <view style="padding-top: 10px" v-for="(item, i) in data.candidateSa" :key="i">
          <!-- <view class="c-card-title">
            <view class="c-flex c-card-tag">工单编号：{{ item.orderNum }}</view>
          </view> -->
          <view class="c-card-content">
            <view class="c-m-b-10">
              <text>工单编号：</text>
...
export default {
  ...
  methods: {
    ...
    punchConfirm(id, sid) {
      debugger;
      common.osg.ajax('wmt/biz/punch/confirm', { id: id, sid: sid }, res => {
        this.page = 1;
        this.dataList = [];
        this.punchToBeConfirmed();
      });
    ...
    punchToBeConfirmed() {
      ...
      common.osg.ajax('wmt/biz/punch/toBeConfirmed', params, res => {
