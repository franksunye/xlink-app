...
var osg = {
   ...
   * @param {Object} callback
   */
  alert: function (msg, callback, title, confirmButtonText, copy) {
    // #ifdef MP-WEIXIN
    wx.showModal({
      title: title || '提示',
      content: msg,
  ...
  confirm: function (msg, callback, options) {
    ...
    wx.showModal({
      ...
      confirmColor: options.confirmColor || '#ff6900',
    ...
    osg._cthis.$dialog
      .confirm({
        ...
        confirmButtonColor: options.confirmColor || '#ff6900',
  ...
  upsertUser(user) {
    ...
    osg.ajax('user/upsertWithJson', { data: JSON.stringify(user) }, res => {}, {
      noload: true
    });
  },
    ...
    wx.uploadFile({
      ...
      success: res => {
        ...
        if (Array.isArray(event.detail.file)) {
          currentFileType = event.detail.file[0].fileType || event.detail.file[0].type;
        } else {
          currentFileType = event.detail.file.type || event.detail.file.fileType;
        }
  ...
  location(callback, setting) {
    ...
    uni.getLocation({
      type: 'gcj02',
      isHighAccuracy: plus.os.name == 'iOS' ? false : true,
      geocode: false,
      success: res => {
  ...
  /**
   * 地理编码
   * 高德地图企业Key：222af85515a49668992817ba7d564425
   * 高德地图个人Key：1544e78c6949e7843efff0691ce9dc67
   */
  ...
  driving(points, callback) {
    osg.ajax(
      `~https://restapi.amap.com/v3/direction/driving?key=1544e78c6949e7843efff0691ce9dc67&origin=${
        points[0].longitude
  ...
  readableTime(val, type) {
    ...
    if (typeof val === 'string' && formattedDatePattern.test(val)) {
      if (type == 'date') {
        ...
        let duration = new Date(val).getTime();
        ...
        events: [
          {
            ...
            app_version: osg.appBaseInfo.appVersion, // plus.runtime.getProperty比uni.getAppBaseInfo()更准确
  ...
  setClipboardData(data, msg) {
    wx.setClipboardData({
      data: data,
      success() {
        if (msg != 'none') osg.toast(msg || '已复制', 'none');
      }
    });
  },
  // 节流
  ...
  keepLive(callback) {
    ...
    // 设置通知栏
    osg.keep.setTitle('吉柿修房app正在后台提供服务'); // 设置通知标题
      ...
      osg.keep.onAddBackgroundCallback(res => {
        ...
        if (res) {
          ...
          osg.keep.startAleraTask(0, times, () => {
            ...
            osg.keep.setTitle('吉柿修房app正在后台提供服务');
  ...
  pushEvent(res) {
    ...
    uni.onPushMessage(res => {
      if (res.type == 'click' && res.data) {
        osg.report('pushClick', res.data);
      }
      if (res.type == 'click' && res.data.payload && res.data.payload.url) {
        // 已打开过工单列表不会再出发onLoad，无法传参
        let str = 'pages/serviceAppointment/serviceAppointments?functionId=';
        if (res.data.payload.url.indexOf(str != -1)) {
          const app = getApp();
          app.globalData.funcId = res.data.payload.url.split(str)[1];
        }
