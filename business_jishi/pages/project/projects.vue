        ...
        <block v-if="index - 1 <= j && j <= index + 1 && list[j]">
          ...
          >
            ...
            <view class="card-view card-buttons">
              <block v-if="currentRole._id == '6631748470755706348'">
                <van-button
                  plain
                  icon="orders-o"
                  block
...
export default {
  ...
  methods: {
    ...
    toDetail(id, index1, index2) {
      ...
      if (this.currentRole._id == '6631748470755706348') {
    ...
    getUserRoles(callback) {
      return new Promise((resolve, reject) => {
        common.osg.ajax('role/queryRoleByPersonInTenant', null, res => {
          let currentRole = res.data.find(item => item._id == '1259263938717893739');
          if (currentRole) {
    ...
    qwMiniLogin(code, callback) {
      common.osg.ajax(
        'index/qwMiniLogin',
        { code, state: '4670873342363649607,ww9da5be52fcf5b613,1000003' },
        async res => {
          // 缓存sessionId
          wx.setStorageSync('sessionId', res.data.token);
          // 当前角色不是工队时
          if (res.data.currentRole._id != '1259263938717893739') {
