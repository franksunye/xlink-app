    ...
    <view class="chargeStandard card">
      ...
      <view v-else>
        ...
        <view style="padding: 3vw">
          ...
          >
            <view>
              专业管家会为您上门进行现场勘查，给出维修方案和并通过专业系统生成电子定制报价。报价由不同维修部位的施工工艺、使用的材料、质保的年限以及相关的工作环节费用组成。您可以现场查看报价单明细或登录“吉柿修房”查看自己的报价信息。
            </view>
            <view>
              【注意事项】
    ...
    <view class="card" style="padding-bottom: 130rpx">
      ...
      <block v-for="(item, i) in faq" :key="i">
        <view class="faq-item" v-if="packup ? i < 1 : true">
          <view style="color: #ff6900" class="c-weight-600">
            <text>Q：</text>
  ...
  methods: {
    ...
    getData() {
      common.osg.ajax('basic/partner/queryById/' + '3904066108544797157', {}, res => {
        ...
        if (res.entity.tenantId == '4670873342363649607') {
