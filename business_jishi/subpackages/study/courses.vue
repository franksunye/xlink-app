<template>
  <view class="courses" @click.capture="track">
    <view class="top">
      <van-search
        placeholder="搜课程/培训"
        shape="round"
      ...
      <van-tabs
        ...
        line-width="25px"
        :ellipsis="false"
        :data-event="{ id: 'YV8TF7koVz', capture: false }"
      >
        <van-tab title="首页" name="recommend"></van-tab>
        <van-tab
          v-for="(item, i) in options.courseType"
          :key="i"
          :title="item.name"
          :name="item.value"
        ></van-tab>
      ...
      <view class="card" v-for="(item, i) in fpi.data" :key="i">
        ...
        <view class="content">
          <view>
            ...
            <view
              class="desc c-ellipsis2"
              @click="toast(item.intro)"
              :data-event="{ id: 'fO8_wt_9ni' }"
            >
              {{ item.intro }}
            </view>
          </view>
  ...
  methods: {
    ...
    getData() {
      common.osg.ajax(
        ...
        {
          _all: '1',
          'is:courseType|string#and': this.activeTab == 'recommend' ? null : this.activeTab,
          'is:state|integer#and': 1,
          'is:exts.learningType|string#and': 'b',
          notFilterBelongTo: '1',
          notFilterOrgId: '1',
