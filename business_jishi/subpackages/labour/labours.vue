<template>
  <view class="labours" @click.capture="track">
    <view class="top">
      <van-search
      ...
      <van-dropdown-menu active-color="#1989fa" custom-class="c-dropdown-menu">
        ...
        <van-dropdown-item :title="itemTitle">
          <van-picker :columns="columns" @change="regionItemChange" />
        </van-dropdown-item>
    ...
    <view class="c-cards">
      <view
        ...
        :data-event="{ id: 'lIU1Zg1z3B' }"
      >
        <view class="c-flex c-flex-between" :data-event="{ id: 'lIU1Zg1z3B' }">
          <view class="c-flex" :data-event="{ id: 'lIU1Zg1z3B' }">
            <!-- #ifdef MP-WEIXIN -->
            <van-image
              width="50px"
              height="50px"
            ...
            <!-- #ifdef APP-PLUS -->
            <image
              :src="item.avatar || 'https://pub.fsgo365.cn/headportrait.png'"
              style="width: 50px; height: 50px; background-color: #fff"
              class="c-card-image"
              :data-event="{ id: 'lIU1Zg1z3B' }"
            ></image>
            <!-- #endif -->
            <view class="c-flex c-flex-col c-flex-between" :data-event="{ id: 'lIU1Zg1z3B' }">
              <view class="c-flex c-item-center" :data-event="{ id: 'lIU1Zg1z3B' }">
                <text style="margin-right: 3px" :data-event="{ id: 'lIU1Zg1z3B' }">
                  {{ item.name }}
                </text>
                <view
                  class="c-tag-warning"
              ...
              <view style="color: #888888; font-size: 14px" :data-event="{ id: 'lIU1Zg1z3B' }">
        ...
        <view
          ...
          style="justify-content: normal; margin-top: 25rpx; display: flex"
          :data-event="{ id: 'lIU1Zg1z3B' }"
        >
          <view
        ...
        <view
          ...
          style="justify-content: normal; margin-top: 5px; display: flex"
          :data-event="{ id: 'lIU1Zg1z3B' }"
        >
          <view
        ...
        <view class="memo" v-if="item.memo" :data-event="{ id: 'lIU1Zg1z3B' }">
          {{ item.memo }}
        </view>
        <view
          style="
        ...
        >
          <view class="c-flex c-item-center" :data-event="{ id: 'lIU1Zg1z3B' }">
            ...
            />
            <!-- #endif -->
            <text style="color: #666666; font-size: 14px" :data-event="{ id: 'lIU1Zg1z3B' }">
...
export default {
  ...
  methods: {
    ...
    //当地区改变的时候触发
    regionItemChange(event) {
      const { picker, value, index } = event.detail;
      picker.setColumnValues(1, citys[value[0]]);
      let LocationName = picker.getValues();
      let valueDetail = '';
      if (LocationName[1]) {
        const matchedItem = this.options.district.find(item => item.text === LocationName[1]);
        valueDetail = matchedItem ? matchedItem.value : null;
        this.itemTitle = `${LocationName[0]}/${LocationName[1]}`;
      } else {
        this.itemTitle = '全部';
      }
      this.filter.district = valueDetail;
      this.getData();
    },
