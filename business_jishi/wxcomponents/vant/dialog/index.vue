...
VantComponent({
    ...
    props: {
        show: {
            ...
            observer(show) {
                ...
                // App端watch监听不触发，所以在此监听
                if(isApp && show && this['data-event']){
                    common.osg.report('dialogOpen', this['data-event']);
                }
            },
        },
        title: String,
    ...
    methods: {
        onConfirm() {
            if(isApp){
                if(this['data-event']) common.osg.report('dialogConfirm', this['data-event']);
            }else if(isMP){
                if(this.dataset && this.dataset.event) common.osg.report('dialogConfirm', this.dataset.event);
            }
            this.handleAction('confirm');
        },
        onCancel() {
            if(isApp){
                if(this['data-event']) common.osg.report('dialogCancel', this['data-event']);
            }else if(isMP){
                if(this.dataset && this.dataset.event) common.osg.report('dialogCancel', this.dataset.event);
            }
    ...
    watch:{
        show(newValue){
            if(isMP && newValue){
                if(this.dataset && this.dataset.event) common.osg.report('dialogOpen', this.dataset.event);
            }
        }
    }
});
