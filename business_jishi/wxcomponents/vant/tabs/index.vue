<template>
<uni-shadow-root class="vant-tabs-index"><view :class="'custom-class '+(utils.bem('tabs'))">
...
VantComponent({
    ...
    methods: {
        ...
        onBeforeChange(index) {
            if(isApp){
                if(this['data-event']) common.osg.report('click', Object.assign({},this['data-event'],{id:this['data-event'].id + '_' + this.data.tabs[this.data.currentIndex].title}));
            }else if(isMP){
                if(this.dataset && this.dataset.event) common.osg.report('click', Object.assign({},this.dataset.event,{id:this.dataset.event.id + '_' + this.data.tabs[this.data.currentIndex].title}));
            }
