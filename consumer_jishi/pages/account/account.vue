    ...
    <view>
      <view class="user-info">
        <view>
          ...
          <view
            v-else
            style="width: 85px; height: 85px; border-radius: 50%; background-color: #ff6900"
          ></view>
          <view class="text">
            <view class="name">
              {{ userInfo.nickname || userInfo.name }}
              <!-- <text v-if="userInfo.phone" class="c-m-l-20">{{ userInfo.phone }}</text> -->
            </view>
          </view>
        </view>
      ...
      <view
        class="order"
        v-if="!userInfo.phone || sa._id"
        @click="
          sa._id &
            open('../../subpackages/serviceAppointment/serviceAppointmentDetail', { id: sa._id })
        "
      >
        <view class="service-top">
          ...
          <view v-if="userInfo.phone" class="c-uni-button text" style="min-width: 0">
        ...
        <view v-if="maintainStr" style="color: #999999; font-size: 12px; margin-left: 6px">
          {{ maintainStr }}
        </view>
        <!-- <view
          ...
          >
            <view style="width: 100%">
              <view
                ...
                style="background: #f6f8fe; border-radius: 12px; padding: 10px 15px"
              >
                <view>
      ...
      <!-- <block v-for="(item, i) in orders" :key="i">
        ...
        >
          ...
          <view class="c-flex c-item-center c-hidden" style="border-radius: 12px">
            <view
      ...
      <view style="display: flex; margin: 10px 15px 0px 15px">
        ...
        <view class="file" style="margin-right: 10px">
          <view>
            <view class="file-title">卡券中心</view>
            <view class="file-content">可用券</view>
            <view style="display: flex; align-items: center; margin-left: 12px">
              <view class="file-number">{{ (myIncomeData.surplus || 0) / 100 }}</view>
              <view class="file-unit">个</view>
            </view>
          </view>
          <view style="text-align: -webkit-center; margin-left: -5px">
            <view class="file-button" @click="open('../../subpackages/coupon/my', {}, 'auth')">
              去使用
            </view>
          </view>
        </view>
      ...
      <view class="c-cell-group">
        ...
        <view
          class="c-cell"
          @click="open('../../subpackages/serviceAppointment/serviceAppointments', {}, 'auth')"
        >
          <text>我的工单</text>
  ...
  onLoad(params) {
    ...
    common.osg.doLogin({}, () => {
      ...
      if (this.userInfo.phone) {
        ...
        // this.getMyIncome();
        this.getCouponPrintingNumber();
        this.getServiceVoucherNumber();
      }
    });
  },
  onShow(params) {
      ...
      common.osg.ajax(
        ...
        res => {
          ...
          if (res.data.length > 0) {
            ...
            if (this.ordersBjProducts.orderList && this.ordersBjProducts.orderList.length > 0) {
              let order = this.ordersBjProducts.orderList[0];
              let str = order.constructionLocation.split(' / ')[0];
              this.maintainStr = `${str}维修 ${order.maintainAreaNum}㎡`;
            }
          }
        }
      );
