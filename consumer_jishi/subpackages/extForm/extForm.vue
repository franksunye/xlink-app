          ...
          <block v-if="!extField.frontEndConfig.mpDetailHide">
            ...
            <block v-if="extField.state == 1">
              ...
              <block v-else-if="extField.type == 'rate'">
                <uni-forms-item :label="extField.nameDisp" :required="extField.required == '1'">
                  <uni-rate
                    :value="formData[extField.name]"
                    :disabled="currentNode.nodeDef.formFieldIdsReadOnly.indexOf(extField._id) != -1"
              ...
              <block v-else-if="extField.type == 'file'">
                <!-- <van-uploader
                    ...
                    @after-read="afterRead($event, extField)"
                    :data-field="extField"
                    @delete="deleteFile($event, extField)"
                    :max-count="
                    ...
                    :disabled="currentNode.nodeDef.formFieldIdsReadOnly.indexOf(extField._id) != -1"
                  /> -->
                <uni-forms-item :label="extField.nameDisp">
                  <uni-file-picker
                    :value="formData[extField.name]"
                    :auto-upload="false"
                    @select="afterRead($event, extField)"
                    @delete="deleteFile($event, extField)"
                    :limit="
                      extField.frontEndConfig.max || extField.frontEndConfig.max == 0
                        ? extField.frontEndConfig.max
              ...
              <block v-else>
                <!-- <van-field
                  ...
                  :readonly="currentNode.nodeDef.formFieldIdsReadOnly.indexOf(extField._id) != -1"
                  :required="extField.required == '1'"
                  :autosize="extField.type == 'textarea'"
                ></van-field> -->
                <uni-forms-item
                  :label="extField.nameDisp"
                  :name="extField.name"
                ...
                >
                  <uni-easyinput
                    ...
                    :type="extField.type == 'textarea' ? 'textarea' : 'text'"
                    placeholder="请输入"
                  ></uni-easyinput>
                </uni-forms-item>
              </block>
...
export default {
  ...
  methods: {
        ...
        await new Promise(resolve => {
          if (cfgArr.length > 0) {
            ...
            common.osg.ajax(
              ...
              {
                jsonPara: params,
                tenantId: tenantId || '4670873342363649607'
              },
              res => {
                if (res) {
                  // res = JSON.parse(res);
    ...
    afterRead(e, f) {
      ...
      common.osg.afterRead(e, this, f);
    },
    // 删除图片
    deleteFile(e, f) {
      this.formData[f.name].splice(e.index, 1);
      this.$forceUpdate();
    },
    /**
     * 点击图片放大
