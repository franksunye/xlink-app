<template>
<uni-shadow-root class="vant-calendar-index"><template v-if="wxTemplateName === 'calendar'">
...
<van-popup v-if="poppable" :custom-class="'van-calendar__popup--'+(position)" close-icon-class="van-calendar__close-icon" :show="show" :round="round" :position="position" :closeable="showTitle || showSubtitle" :close-on-click-overlay="closeOnClickOverlay" @enter="_$self.$parent.$parent[('onOpen')]($event)" @close="_$self.$parent.$parent[('onClose')]($event)" @after-enter="_$self.$parent.$parent[('onOpened')]($event)" @after-leave="_$self.$parent.$parent[('onClosed')]($event)">
