  ...
  return (
    <div className="pb-2">
      ...
      <div className="relative z-10 -mt-9 space-y-4 px-3.5">
        {variant === "task" ? (
          <TaskHomeContent d={d} router={router} />
        ) : (
          <MetricsHomeContent d={d} router={router} />
        )}
      </div>
    </div>
  );
...
function HomeHero({ d }: { d: Dashboard }) {
  return (
      ...
      <section className="xlink-card p-4 pt-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {d.quickActions.map((a) => (
            <TaskQuickActionButton key={a.label} action={a} router={router} />
          ))}
        </div>
      </section>
    </>
  ...
  return (
    <>
      ...
      <section className="xlink-card p-4 pt-5">
        ...
        <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-2">
          <div className="text-center">
            <span className="block text-[13px] text-[#666d7d]">签单额（元）</span>
            <span className="mt-2 block text-3xl font-bold tabular-nums text-[#090f1e]">{d.todayResult.amount}</span>
            <span className="mt-2 text-[11px] font-extrabold text-[#ff4052]">较昨日 +12.5% ↗</span>
          </div>
          <div className="h-28 w-px bg-[#e1e5ec]" />
          <div className="text-center">
            <span className="block text-[13px] text-[#666d7d]">签单数（单）</span>
            <span className="mt-2 block text-3xl font-bold tabular-nums text-[#090f1e]">{d.todayResult.orders}</span>
            <span className="mt-2 text-[11px] font-extrabold text-[#ff4052]">较昨日 +2 ↗</span>
          </div>
        </div>
      </section>
      ...
      <section className="xlink-card p-4 pt-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {d.quickActions.map((a) => (
            <MetricsCompactQuick key={a.label} action={a} router={router} />
          ))}
        </div>
      </section>
    </>
