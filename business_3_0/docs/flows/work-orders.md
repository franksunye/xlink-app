...
3. **任务详情** `/work-orders/[id]`：`getWorkOrder(id)` — 客户信息、`context`（主副行动）、`activities`（`WorkOrderActivity[]`）；响应可含结构化 `appointments[]`（主预约映射到顶层 `appointment`/`timeText` 等）。**v0.1 Web 详情不渲染独立「服务预约」区块**；概念版 `timeline` 字符串列表亦不在 Web 展示。
