<template>
  <view>
          ...
          <van-dropdown-menu active-color="#ee0a24">
            ...
            >
              ...
              <view class="c-flex c-flex-around c-p-20">
                <van-button plain size="small" @click="filterSortingShow = false">关 闭</van-button>
                <van-button
                  plain
                  size="small"
                  @click="
                ...
                <van-button type="info" size="small" @click="application">应 用</van-button>
              </view>
            </van-dropdown-item>
            <van-dropdown-item
    ...
    <view class="c-content">
      ...
      >
        <block v-for="(func, i) in functionList" :key="i">
          ...
          <van-tab :title="func.name" :name="func._id" :info="statusMap[func._id]">
            <van-notice-bar
                  ...
                  >
                    ...
                    <view class="c-card-content">
                      ...
                      <view class="c-flex" v-if="item.data.phone">
                        <text>联系客户：</text>
                        <view @click.native.stop="makeCall(item.data && item.data.phone)">
                          <text>{{ item.data.phone }}</text>
                          <van-icon name="phone-o" class="c-m-l-20" />
                        </view>
                      </view>
                      <view>
                        <text>详细地址：</text>
                      ...
                      <view v-if="item.data.exts && item.data.exts.applyTimeStr">
                        <text>上门时间：</text>
                        <text>{{ item.data.exts.applyTimeStr }}</text>
                      </view>
                      <view v-if="item.data.describe">
                        <text>备注信息：</text>
                        <text>{{ item.data.describe || '' }}</text>
                      </view>
                    </view>
                    <view
                      v-if="
    ...
    >
      <van-button round type="info" @click="open('/subpackages/login/loginAuth')">登 录</van-button>
    </van-empty>
    <!-- 分派资源弹出层 -->
...
export default {
  ...
  methods: {
    onLoad: function (param) {
      ...
      this.doInit(param);
      this.tenantId = wx.getStorageSync('user').tenantId;
      this.usreId = wx.getStorageSync('user')._id;
    },
    doInit(param) {
    ...
    getFunction() {
      if (!wx.getStorageSync('currentRoleFunctions')) return;
      this.dataCount();
      // 补tab页的工单状态值
      let functionList = wx.getStorageSync('currentRoleFunctions').filter(item => {
        return (
          item.action &&
          item.action.indexOf('serviceAppointments') != -1 &&
    ...
    getData: function (type) {
      ...
      params['gte:exts.applyTimeStr|string#and'] = this.dateTimeData.dateTimeStart;
      params['lte:exts.applyTimeStr|string#and'] = this.dateTimeData.dateTimeEnd;
      common.osg.ajax('basic/serviceAppointment/queryWorkflowNode.do', params, res => {
        this.fpi.pages = res.pages;
    ...
    doOpen: function (e) {
      common.osg.open('serviceAppointment', {
