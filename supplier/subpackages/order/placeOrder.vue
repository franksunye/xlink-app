  ...
  >
    <view class="c-p-b-56">
        ...
        <block v-for="(extField, index) in viewExtFields" :key="index">
          <block v-if="extField.state == '1'">
            ...
            <block v-else-if="extField.type == 'button'">
              <van-button
                v-if="!editType || editType == '1' || editType == '2'"
            ...
            <block v-else-if="extField.type == 'addItem'">
              ...
              <view class="c-cell-view" style="border-bottom: 1px solid #ebedf0">
                <block v-for="(item, i) in formData[extField.name]" :key="i">
                  ...
                  >
                    {{
                      commonEvent('codelabel', null, { option: extField.valueOption, value: item })
                    }}
                  </van-tag>
                </block>
              </view>
      ...
      <view class="c-content">
        <!-- 套餐 -->
        <view class="c-selected" v-if="projPackageCount() > 0">
        ...
        <!-- 工序 -->
        <view class="c-selected" v-if="procedureCount() > 0">
          <view class="c-selected-title">
            <text>工序</text>
            <text class="c-size-13">
              分类合计：
        ...
        <!-- 材料 -->
        <view class="c-selected" v-if="materialCount() > 0">
          <view class="c-selected-title">
            <text>材料</text>
            <text class="c-size-13">
              分类合计：
                      ...
                      <view class="c-flex">
                        ...
                        <view class="c-flex c-flex-col c-flex-1">
                          <view style="margin: 0 0 10rpx">
                            ...
                            <text class="desc-tag" v-if="sku.code">编号：{{ sku.code || '' }}</text>
        ...
        <!-- 机措 -->
        <view class="c-selected" v-if="measureCount() > 0">
          <view class="c-selected-title">
            <text>机措</text>
            <text class="c-size-13">
              分类合计：
        ...
        <!-- 自定义增项 -->
        <view class="c-selected" v-if="addItemCount() > 0">
          <view class="c-selected-title">
            <text>自定义增项</text>
            <text class="c-size-13">
              分类合计：
        ...
        <!-- 自定义减项 -->
        <view class="c-selected" v-if="deleteItemCount() > 0">
          <view class="c-selected-title">
            <text>自定义减项</text>
            <text class="c-size-13">
              分类合计：
        ...
        <view class="c-selection c-h-100" v-if="activeTab == 'projPackage'">
          <view class="c-selection-left">
            <view
              ...
              :key="index"
              @click="
                projPackages.type === item.value
                  ? (projPackages.type = '')
                  : (projPackages.type = item.value)
              "
              :class="
                'c-selection-left-item ' +
                (projPackages.type == item.value ? 'c-bg-white c-left-icon-red c-red' : '')
              "
            >
              {{ item.name }}
          ...
          <view class="c-selection-right c-auto">
            <block v-for="(item, index) in projPackages.data" :key="index">
              <view
                ...
                :data-index="index"
                v-if="
                  (projPackages.searchValue
                    ? item.name && item.name.indexOf(projPackages.searchValue) != -1
                    : true) &&
                  (!projPackages.type || projPackages.type == item.type)
                "
        ...
        <view class="c-selection c-h-100" v-if="activeTab == 'procedure'">
          <view class="c-selection-left">
            <view
              ...
              :key="index"
              @click="
                procedures.type === item.value
                  ? (procedures.type = '')
                  : (procedures.type = item.value)
              "
              :class="
                'c-selection-left-item ' +
                (procedures.type == item.value ? 'c-bg-white c-left-icon-red c-red' : '')
              "
            >
              {{ item.name }}
          ...
          <view class="c-selection-right c-auto">
            <block v-for="(item, index) in procedures.data" :key="index">
              <view
                ...
                :data-index="index"
                v-if="
                  (procedures.searchValue
                    ? item.content && item.content.indexOf(procedures.searchValue) != -1
                    : true) &&
                  (!procedures.type ||
                    (item.exts &&
                      item.exts.repairPart &&
                      item.exts.repairPart.indexOf(`&quot;${procedures.type}`) != -1))
                "
                class="c-selection-right-item"
              >
                <view>{{ item.content }}</view>
        ...
        <view class="c-selection c-h-100" v-if="activeTab == 'material'">
          <view class="c-selection-left">
            <view
              ...
              :key="index"
              @click="
                materials.type === item.value
                  ? (materials.type = '')
                  : (materials.type = item.value)
              "
              :class="
                'c-selection-left-item ' +
                (materials.type == item.value ? 'c-bg-white c-left-icon-red c-red' : '')
              "
            >
              {{ item.name }}
              ...
              <block
                v-if="
                  materials.searchValue
                    ? (item.name && item.name.indexOf(materials.searchValue) != -1) ||
                      (item.code && item.code.indexOf(materials.searchValue) != -1)
                    : true
                "
              >
                <view
                  :data-item="item"
                  :data-index="index"
                  v-if="!materials.type || materials.type == item.type"
                  class="c-selection-right-item"
                >
                  <view>{{ item.name }}</view>
                  <view class="c-flex">
        ...
        <view class="c-selection c-h-100" v-if="activeTab == 'measure'">
          <view class="c-selection-right c-auto">
            <block v-for="(item, index) in measures.data" :key="index">
              <view
                v-if="
                  measures.searchValue
                    ? item.content && item.content.indexOf(measures.searchValue) != -1
                    : true
                "
                :data-item="item"
                :data-index="index"
  ...
  methods: {
      ...
      common.osg.ajax(
        ...
        res => {
          ...
          if (common.osg.isJson(res.bjProducts)) {
            ...
            this.projPackages.data = products.projPackages.data;
            this.projPackages.totalAmount = products.projPackages.totalAmount;

            this.procedures.data = products.procedures.data;
            this.procedures.totalAmount = products.procedures.totalAmount;

            this.measures.data = products.measures.data;
            this.measures.totalAmount = products.measures.totalAmount;

            this.addItems = products.addItems;
            this.deleteItems = products.deleteItems;

            // this.materials.data = products.materials.data;
            this.materials.totalAmount = products.materials.totalAmount;

            // 查找所有sku的spu，重新组合数据，products.orderList  materials.data spuid
            let spuIds = [];
            products.materials.data.map(sku => {
              if (spuIds.indexOf(sku.spuid) == -1) {
                spuIds.push(sku.spuid);
              }
            });
            if (spuIds.length > 0) this.getMaterialSPU(spuIds, products.materials);
          }
        }
      );
    },
    ...
    onChange(e) {
      ...
      if (type == 'evaluate') {
        ...
        console.log(this.formExtFields, '---------1111this.formExtFields');
      }
      //视图刷新
      this.$forceUpdate();
    },
    ...
    placeOrder: function (type) {
      ...
      common.osg.confirm('确认要提交吗？', () => {
        // 整理已选套餐、工序、材料、机措、增项、减项
        let projPackages = {
          totalAmount: this.projPackages.totalAmount,
          data: this.projPackages.data.filter(item => {
            if (item.number > 0) {
        ...
        let procedures = {
          totalAmount: this.procedures.totalAmount,
          data: this.procedures.data.filter(item => {
            if (item.number > 0) {
        ...
        let materials = {
          totalAmount: this.materials.totalAmount,
          data: []
        };
        // 提交数据时将spu拆成sku
        this.materials.data.filter(item => {
          if (item.number > 0) {
            if (item.skus && item.skus.length > 0) {
              ...
              item.skus.map(sku => {
                if (sku.number > 0) {
                  ...
                  item.totalAmount += this.amountToFixed(sku.totalAmount);
                  materials.data.push(sku);
                }
              });
            } else {
              ...
              item.amount = item.price;
              item.totalAmount = this.amountToFixed(item.price * item.number);
              materials.data.push(item);
            }
          }
        });

        let measures = {
          totalAmount: this.measures.totalAmount,
          data: this.measures.data.filter(item => {
            if (item.number > 0) {
              item.describe = item.desc;
              item.amount = item.currentGuideUnitPrice;
              item.totalAmount = this.amountToFixed(item.currentGuideUnitPrice * item.number);
        ...
        let test = this.amountToFixed(
          projPackages.totalAmount +
            procedures.totalAmount +
            materials.totalAmount +
        ...
        let productIds = [];
        console.log(JSON.stringify(bjProducts));
    ...
    onSelectionOpen() {
      ...
      if (this.orderId || this.projPackages.data.length == 0) {
        this.getProjPackages();
      }
      if (this.orderId || this.procedures.data.length == 0) {
        this.getProcedures();
      }
      if (this.orderId || this.materials.data.length == 0) {
        this.getMaterials();
      }
      if (this.orderId || this.measures.data.length == 0) {
        this.getMeasures();
      }
    ...
    fillOrder(row, callback) {
      ...
      let projPackageData = this.projPackages.data.filter(item => item.number > 0);
      let procedureData = this.procedures.data.filter(item => item.number > 0);
      let materialData = this.materials.data.filter(item => item.number > 0);
      let measureData = this.measures.data.filter(item => item.number > 0);
      let addItemData = this.addItems.data.filter(item => item.number > 0);
      if (
        projPackageData.length +
      ...
      this.fillField(data => {
        ...
        console.log(this.formExtFields, '-------2222this.formExtFields');
    ...
    fillField(callback) {
      ...
      common.osg.ajax('basic/order/fillField', params, res => {
        ...
        this.projPackages.data.map(item => {
          if (item.number > 0) {
        ...
        common.osg.ajax(
          ...
          result => {
            if (result.data) {
              res.data.agelimit = result.data.years || 0;
            } else {
              common.osg.alert('您当前选择的工序和材料组合没有维护质保年限');
              res.data.agelimit = 0;
            }
      ...
      common.osg.ajax(
        ...
        res => {
          for (let i = 0; i < res.data.length; i++) {
            res.data[i].number = 0;
            // 编辑订单时已选择的套餐覆盖新套餐
            for (let j = 0; j < this.projPackages.data.length; j++) {
              if (res.data[i]._id == this.projPackages.data[j]._id) {
                res.data[i] = common.osg.deepCopy(this.projPackages.data[j]);
                break;
              }
            }
            // 排查删除的材料和工序
          ...
          this.projPackages.data = res.data;
        }
      );
    },
    // 获取工序列表
    getProcedures() {
      common.osg.ajax(
        ...
        res => {
          for (let i = 0; i < res.data.length; i++) {
            res.data[i].number = 0;
            for (let j = 0; j < this.procedures.data.length; j++) {
              if (res.data[i]._id == this.procedures.data[j]._id) {
                res.data[i] = common.osg.deepCopy(this.procedures.data[j]);
                break;
              }
            }
          }
          this.procedures.data = res.data;
        }
      );
    },
    // 获取材料列表
      ...
      common.osg.ajax(
        ...
        res => {
          for (let i = 0; i < res.data.length; i++) {
            ...
            for (let j = 0; j < this.materials.data.length; j++) {
              if (res.data[i]._id == this.materials.data[j]._id) {
                res.data[i] = this.materials.data[j];
                break;
              }
            }
          }
          this.materials.data = res.data;
        }
      );
    },
    // 获取机措列表
    getMeasures() {
      common.osg.ajax(
        ...
        res => {
          for (let i = 0; i < res.data.length; i++) {
            res.data[i].number = 0;
            for (let j = 0; j < this.measures.data.length; j++) {
              if (res.data[i]._id == this.measures.data[j]._id) {
                res.data[i] = this.measures.data[j];
                break;
              }
            }
          }
          this.measures.data = res.data;
        }
      );
    },
    ...
    onSearch(e, type) {
      this[type].searchValue = e.detail;
    },
    tabChange(e) {
    ...
    selectorPlus(item, type) {
      if ((type == 'projPackages' || type == 'procedures') && this.formData.maintainAreaNum) {
        item.number = Number(this.formData.maintainAreaNum);
      } else {
    ...
    calcTotalPrice() {
      let totalPrice = 0;
      // 计算套餐价
      for (let item of this.projPackages.data) {
        if (item.number > 0) {
          // 计算套餐最低起算价
          if (item.price * item.number < item.minimumPrice) {
      ...
      // 计算工序价
      for (let item of this.procedures.data) {
        if (item.number > 0) {
          // 计算工序最低起算价
          if (item.currentGuideUnitPrice * item.number < item.currentMinimumGuideTotalPrice) {
      ...
      // 计算材料
      for (let item of this.materials.data) {
        if (item.number > 0) {
          if (item.skus && item.skus.length > 0) {
            let skuTotalPrice = 0;
            item.skus.map(sku => {
      ...
      // 计算机措
      for (let item of this.measures.data) {
        if (item.number > 0) {
          totalPrice += item.currentGuideUnitPrice * item.number;
        }
      }
    ...
    projPackageTypeCount(type) {
      // 过滤的部位一级
      let count = 0;
      for (let i = 0; i < this.projPackages.data.length; i++) {
        let item = this.projPackages.data[i];
    ...
    projPackageCount(type) {
      ...
      this.projPackages.totalAmount = this.amountToFixed(totalAmount);
      return this.amountToFixed(count);
    },
    procedureTypeCount(type) {
      // 过滤的部位一级
      let count = 0;
      for (let i = 0; i < this.procedures.data.length; i++) {
        let item = this.procedures.data[i];
        if (
          item.number > 0 &&
          item.editType != '2' &&
          item.exts &&
    ...
    procedureCount() {
      ...
      this.procedures.totalAmount = this.amountToFixed(totalAmount);
      return this.amountToFixed(count);
    },
    materialTypeCount(type) {
      // 过滤材料类型
      let count = 0;
      for (let i = 0; i < this.materials.data.length; i++) {
        let item = this.materials.data[i];
        if (item.number > 0 && item.editType != '2' && item.type == type) {
          count += item.number;
        }
      }
    ...
    materialCount() {
      ...
      for (let i = 0; i < this.materials.data.length; i++) {
        ...
        if (item.number > 0 && item.editType != '2') {
          if (item.skus && item.skus.length > 0) {
            let skuTotalAmount = 0;
            item.skus.map(sku => {
              if (sku.number > 0) {
                skuTotalAmount += sku.price * sku.number;
              }
            });
            totalAmount += this.amountToFixed(skuTotalAmount);
          } else {
            totalAmount += this.amountToFixed(item.price * item.number);
          }
          count += item.number;
      ...
      this.materials.totalAmount = this.amountToFixed(totalAmount);
      return this.amountToFixed(count);
    },
    measureCount() {
      let totalAmount = 0;
      let count = 0;
      for (let i = 0; i < this.measures.data.length; i++) {
        let item = this.measures.data[i];
        if (item.number > 0 && item.editType != '2') {
          totalAmount += this.amountToFixed(item.currentGuideUnitPrice * item.number);
          count += item.number;
        }
      }
      this.measures.totalAmount = this.amountToFixed(totalAmount);
      return this.amountToFixed(count);
    },
    addItemCount() {
      let totalAmount = 0;
    ...
    getMaterialSPU(spuIds, materials) {
      common.osg.ajax(
        'basic/material/query',
        {
          _all: '1',
        ...
        res => {
          ...
          // 替换新数组(嵌套)
          materials.data = materials.data.filter(item => !item.spuid).concat(arr);
          // 返显赋值
          this.$set(this, 'materials', materials);
          // this.calcTotalPrice();
        }
      );
    },
