<template>
  <view class="order" style="padding-bottom: 200rpx">
    <view class="detail">
      ...
      <view style="line-height: 44rpx">
        <view class="c-flex c-item-center">
          <text style="min-width: 100px">订单编号：</text>
        ...
        <view class="c-flex c-item-center">
          <text style="min-width: 100px">支付状态：</text>
          <text>{{ codelabel('payState', formData.payState) || '' }}</text>
        </view>
        <view class="c-flex c-item-center">
          <text style="min-width: 100px">联系人姓名：</text>
          <text>{{ formData.ownerName || '' }}</text>
        </view>
        <view class="c-flex c-item-center">
          <text style="min-width: 100px">联系人电话：</text>
          <text>
            {{ formData.ownerPhone || '' }}
          </text>
        </view>
        <view class="c-flex c-item-center">
          ...
          <view v-show="formData.originPrice || formData.originPrice == 0">
        ...
        <view class="c-flex c-item-center" v-if="formData.preferentialAmount">
          <text style="min-width: 100px">优惠金额：</text>
          <view
            class="c-red"
            v-show="formData.preferentialAmount || formData.preferentialAmount == 0"
          >
            - ¥ {{ formData.preferentialAmount }}
          </view>
        </view>
        <view class="c-flex c-item-center">
          <text style="min-width: 100px">实际金额：</text>
          <view v-show="formData.totalPrice || formData.totalPrice == 0" class="c-red">
            ¥ {{ formData.totalPrice }}
          </view>
        </view>
      </view>
          ...
          <view style="margin-left: 26rpx">
            <block v-for="(item, index) in originalOrder.productsDetail" :key="index">
              <block v-if="productShow">
                <view class="c-p-10">
                  <view class="c-flex c-item-center" @click="changeProcedures(true, item)">
                    {{ item.product.name }}
                    <van-icon
                  ...
                  <view v-if="item.proceduresActive" style="color: #999; font-size: 22rpx">
                    ...
                    >
                      <view class="procedures-notLast" style="flex: 3">
                        <block v-for="pItem in item.proceduresList">
    ...
    <view class="c-content" v-if="orderListShow">
      <view class="c-order" v-for="(order, i) in orderList" :key="i">
        <view class="c-order-top">
          ...
          <view v-if="order.constructionSite">施工部位：{{ order.constructionSite }}</view>
          <view class="c-flex c-flex-between c-item-center">
        ...
        <view>
          <view>
            规格：{{ item.material.spec || ''
              ...
              item.material.specUnit
                ? codelabel('unit', item.material.specUnit)
                : item.material.specUnit
            }}
          </view>
          <view>
            单价： ¥{{ item.material.price }}/{{
    ...
    >
      ...
      >
        <van-button
          ...
          round
          custom-class="c-bg-gray-button"
        >
          取消
        </van-button>
      </view>
      <!--  v-if="formData.payState == 0 || formData.exts.isAgain == 1" -->
      <view
