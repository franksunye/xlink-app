<template>
  <view class="c-box" :class="openForm ? 'c-hidden c-h-50rpx' : ''">
        ...
        <van-tab title="处理记录" name="note" style="margin-bottom: 75px">
          <view class="c-card" v-for="(item, index) in note.data" :key="index">
            <view class="c-card-title">
              <view class="c-flex c-flex-item">
                <van-icon custom-class="c-card-icon" name="todo-list-o"></van-icon>
                <text>{{ item.createTime }}</text>
              </view>
            </view>
            <view class="c-p-20" style="overflow: auto">
              ...
              >
                <van-tab title="订单" name="order">
                  ...
                  >
                    ...
                    <view>
                      ...
                      <text class="c-orange" v-if="item.type == '1'">
                        {{
                          commonEvent('codelabel', null, {
                            option: 'processState',
                ...
                <van-tab title="合同" name="contract">
                  <view class="c-card c-relative" v-for="(item, i) in contract.data" :key="i">
                    ...
                    <view>
                      <text>合同金额：¥ {{ item.afterRefundMoney }}</text>
                    </view>
                    <view>
            ...
            >
              <view style="padding-bottom: 140rpx">
                <view v-for="(form, index2) in formList" :key="index2" class="c-card">
                  ...
                  <view style="min-height: 60rpx">
                    <van-button v-if="form.state != -1" @click="removeForm(form)" round>
                      删除
                    </van-button>
                    <view v-else></view>
                    <van-button
    ...
    <view class="c-button-group-bottom" v-if="activeTab.detail.name == 'info'">
      ...
      <!-- <view @click="doEdit" class="c-button-group-item c-m-l-10">
        <view class="c-white">填信息</view>
      ...
      >
        <view class="c-white">{{ changeStatusButton }}</view>
    ...
    <view class="c-button-group-bottom" v-if="activeTab.detail.name == 'note'">
      <view
        ...
        class="c-button-group-item c-m-r-10"
      >
        <view class="c-white">填写处理记录</view>
      </view>
    </view>
      ...
      <view class="quicklyFill-wrapper" v-if="isQuicklyFill">
        <view class="quicklyFill-content">
          <!-- <van-button size="small" :class="groupActionNum[currentNode._id]>1 && isQuicklyFill?'':'hidden'" color="#545c64"
					...
					<van-button custom-style="width: 100%;min-height: 110rpx;"  v-if="isQuicklyFill" custom-class="c-button-group-item c-m-l-10" color="#545c64" @click="fillNext">{{groupActionNum[currentNode._id]==groupLength?'提交':'下一步'}}</van-button>
          <view></view>-->
          <view
            :class="
    ...
    <van-action-sheet :show="reasons.show" :title="reasons.title" class="reason-sheet">
      ...
      <view class="c-flex c-flex-around c-flex-center" style="padding: 15px">
        ...
        <van-button type="info" size="mini" round @click="reasonConfirm">确 认</van-button>
      </view>
    </van-action-sheet>
        ...
        res => {
          ...
          for (let extField of this.extFields) {
            if (extField.type == 'file') {
              if (extField.preset) {
                if (res.data.workOrder[extField.name]) {
                  for (let obj of res.data.workOrder[extField.name]) {
                ...
                res.data.workOrder.exts = res.data.workOrder.exts || {};
                if (res.data.workOrder.exts[extField.name]) {
                  for (let obj of res.data.workOrder.exts[extField.name]) {
                    if (obj.url && obj.url.startsWith('blob:')) {
                      obj.url = obj.url.substring(5);
                    }
                    console.log(obj.url);
