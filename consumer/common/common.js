...
// HBuilderX 运行环境自动切换逻辑
if (process.env.NODE_ENV === 'development') {
  // 开发运行模式：使用测试/Beta地址
  _host = 'https://xlinkbeta.fsgo365.cn';
  // 如需本地调试，可在此处切换为本地 IP 或 localhost
...
var osg = {
      ...
      success: res => {
        uni.login({
          ...
          success: loginRes => {
            ...
            // #ifdef MP-WEIXIN
            var url = 'index/login';
            params['appID'] = 'wxd7991022b6d34a2e';
            // #endif
            // #ifdef MP-ALIPAY
            var url = 'index/aliLogin';
            // #endif
            // #ifdef MP-TOUTIAO
            var url = 'index/dyLogin';
            // params['appID'] = '';
            // #endif
            osg.ajax(
              url,
              ...
              res => {
                ...
                // 缓存sessionId
                uni.setStorageSync('sessionId', res.data.data.token);
    ...
    if (opts) {
      ...
      await new Promise(resolve => {
        if (cfgArr.length > 0) {
          ...
          let url = 'i/code/selectCode.do';
          // #ifdef H5
          url = `~${osg.host}/fsgo/i/code/selectCode.do`;
          // #endif
          let user = wx.getStorageSync('user');
          this.ajax(
            url,
            {
              jsonPara: params,
              tenantId: (user && user.tenantId) || '1340835581184723420'
            },
            res => {
              if (res) {
                // res = JSON.parse(res);
  ...
  afterRead(event, that, field) {
    const tempFiles = event.tempFiles;
    //判断是否为数组，是则循环上传
    if (Array.isArray(tempFiles)) {
      for (let file of tempFiles) {
        osg.fileUpToken(that, file, field);
      }
    } else {
      osg.fileUpToken(that, tempFiles, field);
    }
  },
  //上传
  fileUpToken(that, file, field) {
    osg.ajax(
      ...
      data => {
        ...
        wx.uploadFile({
          ...
          formData: data,
          success: res => {
            let fileName = JSON.parse(res.data).key;
            if (field.preset) {
              that.formData[field.name] = that.formData[field.name] || [];
              that.formData[field.name].push({
                name: fileName,
                extname: file.extname,
                url: osg.downloadUrl + fileName
              });
            } else {
              that.formData.exts[field.name] = that.formData.exts[field.name] || [];
              that.formData.exts[field.name].push({
                name: fileName,
                extname: file.extname,
                url: osg.downloadUrl + fileName
              });
  ...
  getPhoneNumber(e, callback) {
    ...
    if (e.detail.code) {
      let params = {
        code: e.detail.code,
        appID: 'wxd7991022b6d34a2e'
      };
      osg.ajax('index/getPhoneNumber', params, res => {
        osg.toast('授权成功', 'none');
        // 缓存用户信息
  ...
  changePhoneNumber(e, callback) {
    ...
    if (e.detail.code) {
      ...
      osg.ajax('index/changePhone', params, res => {
        osg.toast('修改成功', 'none');
        // 缓存用户信息
        wx.setStorageSync('user', res.data.user);
        if (callback) callback(res.data.user);
  ...
  // 支付宝授权订阅消息弹窗
  requestSubscribeMessage(entityIds, callback) {
    my.requestSubscribeMessage({
      entityIds, // 需要用户订阅的消息模板的id的集合，数组格式
      success: res => {
        if (callback) callback(res);
      },
      fail: res => {
   ...
   * @param {Object|String} options 配置项或场景类型
   */
  getImgUrl(url, options) {
    if (!url || typeof url !== 'string') return '';
    if (url.indexOf('pub.fsgo365.cn') === -1) return url;
    if (url.indexOf('?') !== -1) return url; // 避免重复处理

    // 默认配置
    const defaults = {
      default: 'imageView2/2/w/750/format/webp/q/75',
      thumbnail: 'imageView2/2/w/200/format/webp/q/75', // 列表页缩略图
      avatar: 'imageView2/2/w/100/format/webp/q/75'     // 头像
    };

    let params = defaults.default;
    if (options && typeof options === 'object' && options.type && defaults[options.type]) {
      params = defaults[options.type];
    } else if (typeof options === 'string' && defaults[options]) {
