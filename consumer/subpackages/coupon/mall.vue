  ...
  <view class="mall">
    ...
    <view class="header">
      <view class="type">
        ...
        >
          {{ item.label }}({{
            (item.type == '1' ? fpi.validStockCount : serviceVoucherList.validStockCount) || 0
          }})
        </view>
      </view>
    ...
    <block v-if="type == '2'">
      <!-- 服务券数量大于0 -->
      <view class="content" v-if="serviceVoucherList.data.length > 0">
              ...
              <view class="coupon-left">
                <view class="info">
                  ...
                  <view class="c-size-12">
                    活动时间：{{
                      item.exts.startDate
                        ? item.exts.startDate.split(' ')[0].replace(/-/g, '.')
                        : ''
                    }}-{{
