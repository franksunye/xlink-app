<template>
  <view class="my">
    <view class="header">
      <view class="type">
        <view
          ...
          :key="index"
          :class="item.type == type ? 'c-red' : ''"
          @click="handleClick(item.type)"
        >
          {{ item.label }}({{ item.type == '1' ? fpi.total : serviceVoucherList.total }})
        </view>
      </view>
      <view class="filter" v-if="!privateCouponIds">
        <block v-for="(item, i) in options[tabList[type - 1].options]" :key="i">
          <view
    ...
    <view class="content" v-if="type == '2' && serviceVoucherList.total > 0">
    ...
    <view v-if="type == '1' && fpi.total == 0" class="c-uni-empty">
      <image src="https://pub.fsgo365.cn/empty.png"></image>
      <text>您还没有优惠券，去卡券中心看看吧~</text>
    </view>
    <view v-if="type == '2' && serviceVoucherList.total == 0" class="c-uni-empty">
      <image src="https://pub.fsgo365.cn/empty.png"></image>
      <text>您还没有服务券，去卡券中心看看吧~</text>
    </view>
    <view v-if="type == '3'" class="c-uni-empty">
      <image src="https://pub.fsgo365.cn/empty.png"></image>
      <text>您还没有会员卡，去卡券中心看看吧~</text>
    </view>
    <view class="footer">
