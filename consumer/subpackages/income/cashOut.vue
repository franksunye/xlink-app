<template>
  <view class="account">
    <view class="topBlock">
      <view>
        <view>可提现金额：{{ myIncomeData.surplus / 100 }}</view>
        <view>100积分=1元</view>
      </view>
    </view>
    ...
    <view class="middleBtn" v-if="zhelongState !== '2'">
...
export default {
  ...
  onLoad(params) {
    ...
    this.zhelongState = user.zhelongState || '';
    this.getMyIncome();
  ...
  methods: {
    ...
    handlePrice(e) {
      ...
      e.detail = e.detail.replace(/[^\d.]/g, ''); //清除“数字”和“.”以外的字符
      e.detail = e.detail.replace(/\.{2,}/g, '.'); //只保留第一个. 清除多余的
      e.detail = e.detail.replace('.', '$#$').replace(/\./g, '').replace('$#$', '.');
      e.detail = e.detail.replace(/^(\-)*(\d+)\.(\d\d).*$/, '$1$2.$3'); //只能输入两个小数
      e.detail = e.detail.replace(/^\./g, ''); //首位不能输入“.”
      if (e.detail.indexOf('.') < 0 && e.detail != '') {
        //如果没有小数点，首位不能为0，如01、02...
        e.detail = parseFloat(e.detail);
      }
    ...
    zhelongSing() {
      common.osg.ajax('biz/zhelong/sign', {}, res => {
        debugger;
        this.doOpen('../web/web', {
          url: res.data
        });
    ...
    takeCash(val) {
      ...
      common.osg.ajax('basic/income/takeCash', { type, count }, res => {
